 ...
 */
 ...
 pub extern "C" fn Curl_input_digest(
     ...
     if unsafe{curl_strnequal(
         ...
         || unsafe{Curl_isspace(*header.offset(6 as i32 as isize) as u8 as i32)}
             == 0
     {
         return CURLE_BAD_CONTENT_ENCODING;
     }
     header = unsafe{header.offset(strlen(b"Digest\0" as *const u8 as *const libc::c_char) as isize)};
 ...
 pub extern "C" fn Curl_output_digest(
    ...
    unsafe{curl_dbg_free(
     ...
     unsafe{*allocuserpwd = 0 as *mut libc::c_char;}
     /* not set means empty */
     if userp.is_null() {
   ...
   */
     ...
     if unsafe{ (*authp).iestyle()} != 0 {
         tmp = unsafe{strchr(uripath as *mut libc::c_char, '?' as i32)};
         if !tmp.is_null() {
             let mut urilen: size_t =
             unsafe{tmp.offset_from(uripath as *mut libc::c_char) as size_t};
                 /* typecast is fine here since the value is always less than 32 bits */
             path = unsafe{curl_maprintf(
     ...
     if tmp.is_null() {
         ...
         unsafe{Curl_cstrdup.expect("non-null function pointer")(uripath as *mut libc::c_char)
                 as *mut u8};
         #[cfg(CURLDEBUG)]
         let mut newpath: *mut u8 = unsafe{curl_dbg_strdup(
