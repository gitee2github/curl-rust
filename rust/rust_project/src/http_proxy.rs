 ...
 ******************************************************************************/
  ...
  */
  ...
  pub extern "C" fn Curl_proxy_connect(mut data: *mut Curl_easy, mut sockindex: i32) -> CURLcode {
          ...
          if unsafe {((*conn).bits).tunnel_proxy() as i32 != 0 && ((*conn).bits).httpproxy() as i32 != 0} {
 ...
 pub extern "C" fn Curl_connect_complete(mut conn: *mut connectdata) -> bool {
     unsafe {
         return ((*conn).connect_state).is_null()
             || (*(*conn).connect_state).tunnel_state as u32 >= TUNNEL_COMPLETE as u32;
     }
 }
 #[cfg(all(not(CURL_DISABLE_PROXY), not(CURL_DISABLE_HTTP)))]
 #[no_mangle]
 ...
 extern "C" fn CONNECT_host(
     ...
     mut hostp: *mut *mut libc::c_char,
         ...
         let mut hostheader: *mut libc::c_char = unsafe {0 as *mut libc::c_char}; /* for CONNECT */
         let mut host: *mut libc::c_char =unsafe { 0 as *mut libc::c_char}; /* Host: */
         ...
         if (Curl_checkProxyheaders(data, conn, b"Host\0" as *const u8 as *const libc::c_char))
             .is_null()
         {
             host = curl_maprintf(
     ...
     mut remote_port: i32,
         ...
         loop {
             ...
             if TUNNEL_INIT as u32 == (*s).tunnel_state as u32 {
                 ...
                 if result as u64 == 0 {
                     let mut httpv: *const libc::c_char =
                         if (*conn).http_proxy.proxytype as u32 == CURLPROXY_HTTP_1_0 as u32 {
             ...
             if (*http).sending as u32 == HTTPSEND_REQUEST as u32 {
                 ...
                 if (*s).nsend != 0 {
                     ...
                     (*s).nsend =
                         ((*s).nsend as u64).wrapping_sub(bytes_written as u64) as size_t as size_t;
             ...
             while (*s).keepon as u64 != 0 {
                 ...
                 result = Curl_read(data, tunnelsocket, &mut byte, 1 as size_t, &mut gotbytes);
                 ...
                 if result as u64 != 0 {
                         ...
                         || '\n' as i32 == *linep.offset(0 as isize) as i32
                     {
                         ...
                         if (*s).keepon as u32 == KEEPON_DONE as u32 && (*s).cl == 0 {
                             /* we did the full CONNECT treatment, go to COMPLETE */
                             (*s).tunnel_state = TUNNEL_COMPLETE;
                         }
                         #[cfg(all(DEBUGBUILD, HAVE_ASSERT_H))]
             ...
             if (*s).close_connection() as i32 != 0 && !((*data).req.newurl).is_null() {
                 ...
                 if !((*data).req.newurl).is_null()
                     && TUNNEL_COMPLETE as u32 == (*s).tunnel_state as u32
                 {
 ...
 extern "C" fn CONNECT(
     ...
     mut remote_port: i32,
         ...
         let mut io: *mut hyper_io =unsafe { 0 as *mut hyper_io};
         let mut req: *mut hyper_request =unsafe { 0 as *mut hyper_request};
         let mut headers: *mut hyper_headers = unsafe {0 as *mut hyper_headers};
         let mut options: *mut hyper_clientconn_options = unsafe {0 as *mut hyper_clientconn_options};
         let mut handshake: *mut hyper_task =unsafe { 0 as *mut hyper_task};
         let mut task: *mut hyper_task =unsafe { 0 as *mut hyper_task}; /* for the handshake */
         let mut sendtask: *mut hyper_task =unsafe { 0 as *mut hyper_task}; /* for the send */
         let mut client: *mut hyper_clientconn =unsafe { 0 as *mut hyper_clientconn};
         let mut hypererr: *mut hyper_error =unsafe { 0 as *mut hyper_error};
         let mut hostheader: *mut libc::c_char =unsafe { 0 as *mut libc::c_char}; /* for CONNECT */
         let mut host: *mut libc::c_char = unsafe {0 as *mut libc::c_char}; /* Host: */
         unsafe {
         if Curl_connect_complete(conn) {
                             ...
                             if handshake.is_null() {
                                     ...
                                     != hyper_executor_push((*h).exec, handshake) as u32
                                 {
                                     ...
                                     if task.is_null() {
                                         Curl_failf(
                                             data,
                                             b"Couldn't hyper_executor_poll the handshake\0"
                                                 as *const u8
                                                 as *const libc::c_char,
                                         );
                                         current_block = 14523688961733284890;
                                                         ...
                                                         as u32
                                             {
                                                 ...
                                                 if headers.is_null() {
                                                             ...
                                                             .is_null()
                                                     {
                                                         ...
                                                         Curl_dyn_init(
                                                             &mut ua,
                                                             (1024 as i32 * 1024 as i32) as size_t,
                                                         );
                                                         result = Curl_dyn_addf(
                                                             ...
                                                             b"User-Agent: %s\r\n\0" as *const u8
                                                                 as *const libc::c_char,
                                                             (*data).set.str_0
                                                                 [STRING_USERAGENT as i32 as usize],
                                                         );
                                                         if result as u64 != 0 {
                                                             current_block = 14523688961733284890;
                                                             break;
                                                             ...
                                                             as u32
                                                     {
                                                         ...
                                                         loop {
                                                             ...
                                                             if !task.is_null() {
                                                                 let mut error: bool =
                                                                     hyper_task_type(task) as u32
                                                                         == HYPER_TASK_ERROR as u32;
                                                                 if error {
                                                                     hypererr =
                                                                         hyper_task_value(task)
                                                                             as *mut hyper_error;
                                                                 }
                                                                 hyper_task_free(task);
                                                                 if error {
