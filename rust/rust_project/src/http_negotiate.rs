...
pub extern "C" fn Curl_input_negotiate(
    ...
    unsafe {
        ...
        if proxy {
            match () {
                ...
                _ => {
                    ...
                    service = if !((*data).set.str_0[STRING_PROXY_SERVICE_NAME as i32 as usize])
                        .is_null()
                    {
            ...
            userp = (*conn).user;
            passwdp = (*conn).passwd;
            service = if !((*data).set.str_0[STRING_SERVICE_NAME as i32 as usize]).is_null() {
                (*data).set.str_0[STRING_SERVICE_NAME as i32 as usize] as *const libc::c_char
            } else {
                b"HTTP\0" as *const u8 as *const libc::c_char
            };
            host = (*conn).host.name;
        ...
        header = header.offset(strlen(b"Negotiate\0" as *const u8 as *const libc::c_char) as isize);
        while *header as i32 != 0 && Curl_isspace(*header as u8 as i32) != 0 {
            header = header.offset(1);
        }
        len = strlen(header);
        (*neg_ctx).set_havenegdata((len != 0 as i32 as u64) as i32 as bit);
        ...
        result =
            Curl_auth_decode_spnego_message(data, userp, passwdp, service, host, header, neg_ctx);
        if result as u64 != 0 {
            Curl_http_auth_cleanup_negotiate(conn);
        }
        return result;
...
pub extern "C" fn Curl_output_negotiate(
    ...
    let mut neg_ctx: *mut negotiatedata = if proxy as i32 != 0 {
        unsafe { &mut (*conn).proxyneg }
    } else {
        unsafe { &mut (*conn).negotiate }
    };
    let mut authp: *mut auth = if proxy as i32 != 0 {
        unsafe { &mut (*data).state.authproxy }
    } else {
        unsafe { &mut (*data).state.authhost }
    };
    ...
    if unsafe { *state as u32 } == GSS_AUTHRECV as i32 as u32 {
        ...
        if unsafe { (*neg_ctx).havenoauthpersist() } == 0 {
            unsafe {
                (*neg_ctx)
                    .set_noauthpersist(((*neg_ctx).havemultiplerequests() == 0) as i32 as bit);
            }
        }
    }
            ...
            && unsafe { *state as u32 } != GSS_AUTHSUCC as i32 as u32
    {
        ...
        if unsafe { (*neg_ctx).status } == 0 as u32
            || unsafe { (*neg_ctx).status } == ((1 as i32) << 0 as i32 + 0 as i32) as u32
        {
            unsafe {
                *state = GSS_AUTHDONE;
            }
