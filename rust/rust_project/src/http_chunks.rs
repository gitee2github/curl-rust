...
pub extern "C" fn Curl_httpchunk_read(
    ...
    if unsafe { ((*data).set).http_te_skip() as i32 } != 0 && unsafe { (*k).ignorebody() } == 0 {
        result =
            unsafe { Curl_client_write(data, (1 as i32) << 0 as i32, datap, datalen as size_t) };
        if result as u64 != 0 {
            unsafe {
                *extrap = result;
            }
            ...
            match (*ch).state as u32 {
                ...
                2 => {
                    ...
                    *wrote = (*wrote as u64).wrapping_add(piece) as size_t;
                    (*ch).datasize = ((*ch).datasize as u64).wrapping_sub(piece) as curl_off_t;
                ...
                5 => {
                    if *datap as i32 == 0xd as i32 || *datap as i32 == 0xa as i32 {
