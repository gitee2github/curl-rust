 ...
 ******************************************************************************/
 ...
 extern "C" fn gtls_init() -> i32 {
         ...
         if unsafe{!gtls_inited} {
             ret = if unsafe{gnutls_global_init()} != 0 {
                 0 as i32
             } else {
                 1 as i32
             };
             ...
             if !(fseek(f, 0 as i64, 2 as i32) != 0 as i32
                 ...
                 || {
                     match () {
                         ...
                         _ => {
                             ptr =
                                 Curl_cmalloc.expect("non-null function pointer")(filelen as size_t);
                         }
                         #[cfg(CURLDEBUG)]
                         _ => {
                             ptr = curl_dbg_malloc(
 ...
 extern "C" fn unload_file(mut data: gnutls_datum_t) {
         #[cfg(not(CURLDEBUG))]
         unsafe{Curl_cfree.expect("non-null function pointer")(data.data as *mut libc::c_void);}
 ...
 extern "C" fn handshake(
     ...
     mut nonblocking: bool,
         ...
         loop {
             ...
             timeout_ms = unsafe{Curl_timeleft(data, 0 as *mut curltime, duringconnect)};
             ...
             if unsafe{(*connssl).connecting_state as u32} == ssl_connect_2_reading as u32
                 || unsafe{(*connssl).connecting_state as u32} == ssl_connect_2_writing as u32
             {
                 let mut what: i32 = 0;
             ...
             rc = unsafe{gnutls_handshake(session)};
             if rc == -(28 as i32) || rc == -(52 as i32) {
             ...
             } else if rc < 0 as i32 && unsafe{gnutls_error_is_fatal(rc) }== 0 {
                 let mut strerr: *const libc::c_char = 0 as *const libc::c_char;
                 if rc == -(16 as i32) {
                     let mut alert: i32 = unsafe{gnutls_alert_get(session) as i32};
                     strerr = unsafe{gnutls_alert_get_name(alert as gnutls_alert_description_t)};
                 }
                 if strerr.is_null() {
                     strerr = unsafe{gnutls_strerror(rc)};
                 }
                 unsafe{Curl_infof(
                     data,
                 ...
                 if rc < 0 as i32 {
                     ...
                     if rc == -(12 as i32) {
                         let mut alert_0: i32 = unsafe{gnutls_alert_get(session) as i32};
                         strerr_0 = unsafe{gnutls_alert_get_name(alert_0 as gnutls_alert_description_t)};
                     }
                     if strerr_0.is_null() {
                         strerr_0 = unsafe{gnutls_strerror(rc)};
                     }
                     unsafe{Curl_failf(
                         data,
 ...
 extern "C" fn do_file_type(mut type_0: *const libc::c_char) -> gnutls_x509_crt_fmt_t {
 ...
 extern "C" fn set_ssl_version_min_max(
     ...
     mut tls13support: *const libc::c_char,
         ...
         unsafe{let mut ssl_version: i64 = if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
         ...
         unsafe{let mut ssl_version: i64 = (*conn).ssl_config.version;}
 
         #[cfg(not(CURL_DISABLE_PROXY))]
         unsafe{let mut ssl_version_max: i64 = if CURLPROXY_HTTPS as u32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         unsafe{let mut ssl_version_max: i64 = (*conn).ssl_config.version_max;
 ...
 extern "C" fn gtls_connect_step1(
     mut data: *mut Curl_easy,
     mut conn: *mut connectdata,
     mut sockindex: i32,
 ) -> CURLcode {
         ...
         let mut rc: i32 = 0;
         let mut sni: bool = 1 as i32 != 0; /* default is SNI enabled */
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let hostname: *const libc::c_char = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let certverifyresult: *mut i64 = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let certverifyresult: *mut i64 =unsafe{ &mut (*data).set.ssl.certverifyresult};
         ...
         unsafe{*certverifyresult = 0 as i64;}
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_CONN_CONFIG_version = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         if SSL_CONN_CONFIG_version == CURL_SSLVERSION_SSLv2 as i64 {
             ...
             if SSL_CONN_CONFIG_version == CURL_SSLVERSION_SSLv3 as i64 {
                 sni = 0 as i32 != 0; /* SSLv3 has no SNI */
             }
         }
         /* allocate a cred struct */
         rc = unsafe{gnutls_certificate_allocate_credentials(&mut (*backend).cred)};
         if rc != 0 as i32 {
             unsafe{ Curl_failf(
                 data,
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_SET_OPTION_authtype = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let SSL_SET_OPTION_authtype = unsafe{(*data).set.ssl.authtype as u32};
         #[cfg(HAVE_GNUTLS_SRP)]
         if SSL_SET_OPTION_authtype == CURL_TLSAUTH_SRP as u32 {
             ...
             #[cfg(not(CURL_DISABLE_PROXY))]
             if true {
                 unsafe{rc = gnutls_srp_set_client_credentials(
             ...
             if rc != 0 as i32 {
                 unsafe{Curl_failf(
                     data,
                     b"gnutls_srp_set_client_cred() failed: %s\0" as *const u8
                         as *const libc::c_char,
                     gnutls_strerror(rc),
                 );}
         ...
         if !if CURLPROXY_HTTPS as u32 == unsafe{(*conn).http_proxy.proxytype as u32}
             && ssl_connection_complete as u32
                 != unsafe{(*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
                 } else {
                     1 as i32
                 }) as usize]
         ...
         .is_null()
         {
             unsafe{gnutls_certificate_set_verify_flags((*backend).cred, 0 as u32);
             ...
             if rc < 0 as i32 {
                 ...
                 {
                     unsafe{*certverifyresult = rc as i64;}
         ...
         // #[cfg(CURL_DISABLE_PROXY)]
         if unsafe{!((*conn).ssl_config.CAfile).is_null()} {
             ...
             if rc < 0 as i32 {
                 ...
                 if unsafe{((*conn).ssl_config).verifypeer() != 0} {
                     unsafe{ *certverifyresult = rc as i64;}
                     return CURLE_SSL_CACERT_BADFILE;
                 }
             } else {
         ...
         .is_null()
         {
             rc =unsafe{gnutls_certificate_set_x509_trust_dir(
                 (*backend).cred,
                 if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
                     && ssl_connection_complete as u32
                         != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
         ...
         // #[cfg(CURL_DISABLE_PROXY)]
         if unsafe{!((*conn).ssl_config.CApath).is_null()} {
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         if !if CURLPROXY_HTTPS as u32 == unsafe{(*conn).http_proxy.proxytype as u32}
             && unsafe{ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
                 } else {
                     1 as i32
         ...
         .is_null()
         {
             rc = unsafe{gnutls_certificate_set_x509_crl_file(
                 (*backend).cred,
                 if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
                     && ssl_connection_complete as u32
                         != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
         ...
         // #[cfg(CURL_DISABLE_PROXY)]
         if unsafe{!((*data).set.ssl.CRLfile).is_null()} {
         ...
         rc = unsafe{gnutls_init(&mut (*backend).session, init_flags)};
         if rc != 0 as i32 {
             unsafe{Curl_failf(
                 data,
         ...
         /* convenient assign */
         session = unsafe{(*backend).session};
         // done - 条件编译 - 542
         #[cfg(ENABLE_IPV6)]
         if 0 as i32
             == unsafe{inet_pton(
         ...
         rc = unsafe{gnutls_set_default_priority(session)};
         if rc != 0 as i32 {
             return CURLE_SSL_CONNECT_ERROR;
         }
         /* "In GnuTLS 3.6.5, TLS 1.3 is enabled by default" */
         tls13support = unsafe{gnutls_check_version(b"3.6.5\0" as *const u8 as *const libc::c_char)};
         ...
         if SSL_SET_OPTION_authtype as u32 == CURL_TLSAUTH_SRP as i32 as u32 {
             let mut len: size_t = unsafe{strlen(prioritylist)};
             #[cfg(not(CURLDEBUG))]
             let mut prioritysrp: *mut libc::c_char = unsafe{Curl_cmalloc
                 .expect("non-null function pointer")(
                 len.wrapping_add(::std::mem::size_of::<[libc::c_char; 5]>() as u64)
                     .wrapping_add(1 as u64),
             ) as *mut libc::c_char};
             #[cfg(CURLDEBUG)]
             let mut prioritysrp: *mut libc::c_char = unsafe{curl_dbg_malloc(
                 len.wrapping_add(::std::mem::size_of::<[libc::c_char; 5]>() as u64)
                     .wrapping_add(1 as u64),
                 591 as i32,
                 b"vtls/gtls.c\0" as *const u8 as *const libc::c_char,
             ...
             unsafe{strcpy(prioritysrp, prioritylist);
             strcpy(
             ...
             #[cfg(not(CURLDEBUG))]
             unsafe{Curl_cfree.expect("non-null function pointer")(prioritysrp as *mut libc::c_void);}
             ...
             rc = unsafe{gnutls_priority_set_direct(session, prioritylist, &mut err)};
         }
         ...
         if unsafe{((*conn).bits).tls_enable_alpn() != 0 }{
             ...
             // done - 623
             #[cfg(not(CURL_DISABLE_PROXY))]
             let CURL_DISABLE_PROXY_flag = unsafe{(!(CURLPROXY_HTTPS as u32
                 == (*conn).http_proxy.proxytype as u32
                 && ssl_connection_complete as u32
                     != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                         0 as i32
             ...
             gnutls_alpn_set_protocols(session, protocols.as_mut_ptr(), cur as u32, 0 as u32);}}
         }
         #[cfg(not(CURL_DISABLE_PROXY))]
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_SET_OPTION_key_passwd = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_SET_OPTION_key = unsafe{if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
                 } else {
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_SET_OPTION_cert_type = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let SSL_SET_OPTION_cert_type = unsafe{(*data).set.ssl.cert_type};
         unsafe{
         if !SSL_SET_OPTION_primary_clientcert.is_null() {
             if !SSL_SET_OPTION_key_passwd.is_null() {
                 let supported_key_encryption_algorithms: u32 = (GNUTLS_PKCS_PKCS12_3DES as i32
                     ...
                     | GNUTLS_PKCS_PBES2_AES_256 as i32)
                     as u32;
         ...
         #[cfg(HAVE_GNUTLS_SRP)]
         if SSL_SET_OPTION_authtype == CURL_TLSAUTH_SRP as u32 {
             rc = unsafe{gnutls_credentials_set(
                 session,
                 GNUTLS_CRD_SRP,
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_CONN_CONFIG_verifystatus = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         if SSL_CONN_CONFIG_verifystatus != 0 {
             rc = unsafe{gnutls_ocsp_status_request_enable_client(
                 session,
         ...
         to speed up things */
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_SET_OPTION_primary_sessionid = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         if SSL_SET_OPTION_primary_sessionid != 0 {
             ...
             #[cfg(CURL_DISABLE_PROXY)]
             let SSL_IS_PROXY_void_1 = if 0 as i32 != 0 { 1 as i32 } else { 0 as i32 } != 0;
 ...
 extern "C" fn pkp_pin_peer_pubkey(
     ...
     mut pinnedpubkey: *const libc::c_char,
             ...
             if !(ret != -(51 as i32) || len1 == 0 as u64) {
                unsafe{
                 match () {
                     #[cfg(not(CURLDEBUG))]
                     _ => {
                         buff1 = Curl_cmalloc.expect("non-null function pointer")(len1) as *mut u8;
                     }
                     #[cfg(CURLDEBUG)]
                     _ => {
                 ...
                 if !buff1.is_null() {
                     len2 = len1;
                     ret = unsafe{gnutls_pubkey_export(
                         key,
                         GNUTLS_X509_FMT_DER,
         ...
         if !key.is_null() {
            unsafe{ gnutls_pubkey_deinit(key);}
         }
         #[cfg(not(CURLDEBUG))]
 ...
 extern "C" fn gtls_connect_step3(
     ...
     mut sockindex: i32,
         ...
         let mut certname: [libc::c_char; 65] =unsafe{ *::std::mem::transmute::<
         &[u8; 65],
         &mut [libc::c_char; 65],
     >(
         ...
         let mut bits: u32 = 0;
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         let mut version: gnutls_protocol_t = unsafe{gnutls_protocol_get_version(session)};
         #[cfg(not(CURL_DISABLE_PROXY))]
         let hostname: *const libc::c_char = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
         ...
         ptr = unsafe{gnutls_cipher_suite_get_name(
         ...
         chainp = unsafe{gnutls_certificate_get_peers(session, &mut cert_list_size)};
         unsafe{
         #[cfg(not(CURL_DISABLE_PROXY))]
         if chainp.is_null() {
             if (if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
         ...
         if unsafe{((*data).set.ssl).certinfo() as i32 }!= 0 &&unsafe{ !chainp.is_null()} {
             let mut i: u32 = 0;
             ...
             while i < cert_list_size {
                 ...
                 unsafe{(*chainp.offset(i as isize)).data as *const libc::c_char};
                 let mut end: *const libc::c_char =
                 unsafe{ beg.offset((*chainp.offset(i as isize)).size as isize)};
                 result = unsafe{Curl_extract_certinfo(data, i as i32, beg, end)};
                 if result as u64 != 0 {
                     return result;
                 }
                 i = i.wrapping_add(1);
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_CONN_CONFIG_verifypeer =unsafe{ if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         if SSL_CONN_CONFIG_verifypeer != 0 {
             ...
             rc = unsafe{gnutls_certificate_verify_peers2(session, &mut verify_status)};
             ...
             unsafe{*certverifyresult = verify_status as i64;}
             ...
             if verify_status & GNUTLS_CERT_INVALID as u32 != 0 {
                 if SSL_CONN_CONFIG_verifypeer != 0 {
                    unsafe{
                     ...
                     Curl_failf(
                         data,
                         b"server certificate verification failed. CAfile: %s CRLfile: %s\0"
                             as *const u8 as *const libc::c_char,
                         if !if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
                             && ssl_connection_complete as u32
                                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_CONN_CONFIG_verifystatus_1 = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let SSL_CONN_CONFIG_verifystatus_1 = unsafe{((*conn).ssl_config).verifystatus()};
         unsafe{
         if SSL_CONN_CONFIG_verifystatus_1 != 0 {
            
             if gnutls_ocsp_status_request_is_checked(session, 0 as u32) == 0 as u32 {
                 ...
                 match status as u32 {
                     ...
                     1 => {
                         ...
                         match reason as u32 {
                             1 => {
                                 crl_reason = b"private key compromised\0" as *const u8
                                     as *const libc::c_char;
                             }
                             2 => {
                                 crl_reason =
                                     b"CA compromised\0" as *const u8 as *const libc::c_char;
                             }
                             3 => {
                                 crl_reason = b"affiliation has changed\0" as *const u8
                                     as *const libc::c_char;
                             }
                             4 => {
                                 crl_reason =
                             ...
                             10 => {
                                 crl_reason =
                                     b"AA compromised\0" as *const u8 as *const libc::c_char;
                             }
                             0 | _ => {
                                 crl_reason =
                         ...
                         Curl_failf(
                             data,
                             b"Server certificate was revoked: %s\0" as *const u8
                                 as *const libc::c_char,
                             crl_reason,
                         );
                     }
         ...
         unsafe{ gnutls_x509_crt_init(&mut x509_cert);}
         if !chainp.is_null() {
             /* convert the given DER or PEM encoded Certificate to the native
             gnutls_x509_crt_t format */
             unsafe{ gnutls_x509_crt_import(x509_cert, chainp, GNUTLS_X509_FMT_DER);}
         }
         unsafe{
         #[cfg(not(CURL_DISABLE_PROXY))]
         if !if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
        ...
        unsafe{
         ...
         rc = unsafe{gnutls_x509_crt_get_dn_by_oid(
             x509_cert,
         ...
         failure. */
         rc = unsafe{gnutls_x509_crt_check_hostname(x509_cert, hostname) as i32};
         ...
         if rc == 0 {
             #[cfg(not(CURL_DISABLE_PROXY))]
             let SSL_HOST_DISPNAME_void = unsafe{if CURLPROXY_HTTPS as u32
                 == (*conn).http_proxy.proxytype as u32
                 && ssl_connection_complete as u32
                     != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                         0 as i32
             ...
             #[cfg(CURL_DISABLE_PROXY)]
             let SSL_HOST_DISPNAME_void = unsafe{(*conn).host.dispname};
             #[cfg(not(CURL_DISABLE_PROXY))]
             let SSL_CONN_CONFIG_verifyhost = unsafe{if CURLPROXY_HTTPS as u32
                 == (*conn).http_proxy.proxytype as u32
                 && ssl_connection_complete as u32
                     != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                         0 as i32
             ...
             if SSL_CONN_CONFIG_verifyhost != 0 {
                ...
                unsafe{Curl_infof(
                     data,
                     b"  common name: %s (does not match '%s')\0" as *const u8
                         as *const libc::c_char,
                     certname.as_mut_ptr(),
                     SSL_HOST_DISPNAME_void,
                 );}
         ...
         certclock = unsafe{gnutls_x509_crt_get_expiration_time(x509_cert)};
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_CONN_CONFIG_verifypeer_1 = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let SSL_CONN_CONFIG_verifypeer_1 = unsafe{((*conn).ssl_config).verifypeer()};
         if certclock == -(1 as i32) as time_t {
             if SSL_CONN_CONFIG_verifypeer_1 != 0 {
                unsafe{Curl_failf(
                     data,
                     b"server cert expiration date verify failed\0" as *const u8
                         as *const libc::c_char,
                 );
         ...
         } else if certclock < unsafe{time(0 as *mut time_t) }{
             if SSL_CONN_CONFIG_verifypeer_1 != 0 {
                unsafe{Curl_failf(
                     data,
         ...
         certclock = unsafe{gnutls_x509_crt_get_activation_time(x509_cert)};
         if certclock == -(1 as i32) as time_t {
             if SSL_CONN_CONFIG_verifypeer_1 != 0 {
                unsafe{ Curl_failf(
                     data,
                     b"server cert activation date verify failed\0" as *const u8
                         as *const libc::c_char,
                 );
         ...
         #[cfg(not(CURL_DISABLE_PROXY))]
         let SSL_PINNED_PUB_KEY_void = unsafe{if CURLPROXY_HTTPS as u32
             == (*conn).http_proxy.proxytype as u32
             && ssl_connection_complete as u32
                 != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                     0 as i32
         ...
         */
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         algo = unsafe{gnutls_x509_crt_get_pk_algorithm(x509_cert, &mut bits) as u32}; /* public key algorithm's parameters */
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         unsafe{Curl_infof(
             data,
         ...
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         rc = unsafe{gnutls_x509_crt_get_dn2(x509_cert, &mut certfields)};
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         if rc != 0 {
            unsafe{ Curl_infof(
                 data,
            ...
            unsafe{Curl_infof(
             ...
             unsafe{gnutls_free.expect("non-null function pointer")(certfields.data as *mut libc::c_void);}
         }
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         rc = unsafe{gnutls_x509_crt_get_issuer_dn2(x509_cert, &mut certfields)};
         #[cfg(not(CURL_DISABLE_VERBOSE_STRINGS))]
         if rc != 0 {
            unsafe{ Curl_infof(
                 data,
         ...
         unsafe{gnutls_x509_crt_deinit(x509_cert);}
         if unsafe{((*conn).bits).tls_enable_alpn()} != 0 {
             rc = unsafe{gnutls_alpn_get_selected_protocol(session, &mut proto)};
             if rc == 0 as i32 {
                unsafe{Curl_infof(
                     data,
                 ...
                 // done - 1254
                 #[cfg(USE_HTTP2)]
                 let USE_HTTP2_flag = unsafe{proto.size == 2 as u32
                     && memcmp(
         ...
         unsafe{ (*conn).ssl[sockindex as usize].state = ssl_connection_complete;
         ...
         if SSL_SET_OPTION_primary_sessionid != 0 {
             ...
             unsafe{gnutls_session_get_data(session, 0 as *mut libc::c_void, &mut connect_idsize);}
             /* get a buffer for it */
             unsafe{
             match () {
                 #[cfg(not(CURLDEBUG))]
             ...
             if !connect_sessionid.is_null() {
                 ...
                 unsafe{gnutls_session_get_data(session, connect_sessionid, &mut connect_idsize);}
                 Curl_ssl_sessionid_lock(data);
                 unsafe{
                 #[cfg(not(CURL_DISABLE_PROXY))]
                 let SSL_IS_PROXY_void_1 = if CURLPROXY_HTTPS as u32
 ...
 */
 extern "C" fn gtls_connect_common(
     mut data: *mut Curl_easy,
     mut conn: *mut connectdata,
     mut sockindex: i32,
     mut nonblocking: bool,
     mut done: *mut bool,
         ...
         unsafe{*done = ssl_connect_1 as u32 == (*connssl).connecting_state as u32;}
         return CURLE_OK;
 }
 extern "C" fn gtls_connect_nonblocking(
     mut data: *mut Curl_easy,
     mut conn: *mut connectdata,
     mut sockindex: i32,
     mut done: *mut bool,
 ...
 extern "C" fn gtls_send(
     ...
     mut curlcode: *mut CURLcode,
         ...
         if rc < 0 as i64 {
            unsafe{ *curlcode = (if rc == -(28 as i32) as i64 {
                 CURLE_AGAIN as i32
             } else {
 ...
 extern "C" fn close_one(mut connssl: *mut ssl_connect_data) {
         ...
         if unsafe{!((*backend).cred).is_null()} {
            unsafe{
 ...
 extern "C" fn gtls_close(mut data: *mut Curl_easy, mut conn: *mut connectdata, mut sockindex: i32) {
     unsafe {
 ...
 extern "C" fn gtls_recv(
     ...
     mut curlcode: *mut CURLcode,
         ...
         ret = unsafe{gnutls_record_recv((*backend).session, buf as *mut libc::c_void, buffersize)};
         ...
         if ret == -(37 as i32) as i64 {
             ...
             let mut result: CURLcode = handshake(data, conn, num, 0 as i32 != 0, 0 as i32 != 0);
             if result as u64 != 0 {
                 /* handshake() writes error message on its own */
                 unsafe{ *curlcode = result;}
             } else {
                unsafe{ *curlcode = CURLE_AGAIN;} /* then return as if this was a wouldblock */
             }
             return -(1 as i32) as ssize_t;
         }
 ...
 extern "C" fn gtls_session_free(mut ptr: *mut libc::c_void) {
 ...
 extern "C" fn gtls_version(mut buffer: *mut libc::c_char, mut size: size_t) -> size_t {
     unsafe {
 ...
 extern "C" fn gtls_random(
     ...
     mut length: size_t,
         ...
         let mut rc: i32 = 0;
         rc = unsafe{gnutls_rnd(GNUTLS_RND_RANDOM, entropy as *mut libc::c_void, length)};
         return (if rc != 0 {
 ...
 extern "C" fn gtls_cert_status_request() -> bool {
     unsafe {
         return 1 as i32 != 0;
     }
 }
 extern "C" fn gtls_get_internals(
 ...
 #[no_mangle]
 pub static mut Curl_ssl_gnutls: Curl_ssl = Curl_ssl {
     info: {
         curl_ssl_backend {
     ...
     version: Some(gtls_version as unsafe extern "C" fn(*mut libc::c_char, size_t) -> size_t),
     check_cxn: Some(Curl_none_check_cxn as unsafe extern "C" fn(*mut connectdata) -> i32),
     shut_down: Some(
         gtls_shutdown as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, i32) -> i32,
     ),
     data_pending: Some(gtls_data_pending as unsafe extern "C" fn(*const connectdata, i32) -> bool),
     random: Some(gtls_random as unsafe extern "C" fn(*mut Curl_easy, *mut u8, size_t) -> CURLcode),
     cert_status_request: Some(gtls_cert_status_request as unsafe extern "C" fn() -> bool),
     connect_blocking: Some(
         gtls_connect as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, i32) -> CURLcode,
     ),
     ...
     close_one: Some(
         gtls_close as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, i32) -> (),
     ),
     ...
     sha256sum: Some(
         gtls_sha256sum as unsafe extern "C" fn(*const u8, size_t, *mut u8, size_t) -> CURLcode,
     ),
