 ...
 ******************************************************************************/
 ...
 extern "C" fn blobdup(
     ...
     if !src.is_null() {
       ...
       handle that passed in the blob. */
         ...
         (*d).data = (d as *mut libc::c_char)
             .offset(::std::mem::size_of::<curl_blob>() as isize)
             as *mut libc::c_void;
 ...
 extern "C" fn blobcmp(mut first: *mut curl_blob, mut second: *mut curl_blob) -> bool {
     unsafe{
     if first.is_null() && second.is_null() {/* both are NULL */
         return 1 as i32 != 0;
     }
     if first.is_null() || second.is_null() { /* one is NULL */
         return 0 as i32 != 0;
     }
     if (*first).len != (*second).len {/* different sizes */
         return 0 as i32 != 0;
     }
     return memcmp((*first).data, (*second).data, (*first).len) == 0;/* same data */
     }
 }
 ...
 #[no_mangle]
 pub extern "C" fn Curl_free_primary_ssl_config(
 ...
 */
 ...
 #[cfg(USE_SSL)]
 #[no_mangle]
 pub extern "C" fn Curl_ssl_cleanup() {/* Global cleanup */
     unsafe{
     if init_ssl { /* only cleanup if we did a previous init */
 ...
 extern "C" fn ssl_prefs_check(mut data: *mut Curl_easy) -> bool {
    /* check for CURLOPT_SSLVERSION invalid parameter value */
     ...
     if sslver < 0 as i64
         || sslver >= CURL_SSLVERSION_LAST as i64
     {
         Curl_failf(
             data,
 ...
 */ 
     ...
     #[cfg(CURL_DISABLE_PROXY)]
     let mut port: i32 = (*conn).remote_port;
 ...
 */
     ...
     store = &mut *((*data).state.session).offset(0 as isize)
         as *mut Curl_ssl_session;
     oldest_age = (*((*data).state.session).offset(0 as isize)).age;
     #[cfg(all(DEBUGBUILD, HAVE_ASSERT_H))]
     if (if CURLPROXY_HTTPS as libc::c_uint
         == (*conn).http_proxy.proxytype as libc::c_uint
         && ssl_connection_complete as libc::c_uint
     ...
     if i == (*data).set.general_ssl.max_ssl_sessions {
         ...
         store = &mut *((*data).state.session).offset(i as isize)
             as *mut Curl_ssl_session;/* use this slot */
     }
 ...
 pub extern "C" fn Curl_ssl_close_all(mut data: *mut Curl_easy) {
     unsafe{
     if !((*data).state.session).is_null()
         && !(SSLSESSION_SHARED(data))
     {/* kill the session ID cache if not shared */
         let mut i: size_t = 0;
         i = 0 as size_t;
         while i < (*data).set.general_ssl.max_ssl_sessions {/* the single-killer function handles empty table slots */
 ...
 pub extern "C" fn Curl_ssl_getsock(
     ...
     let mut connssl: *mut ssl_connect_data = &mut *((*conn).ssl)
         .as_mut_ptr()
         .offset(0 as isize) as *mut ssl_connect_data;/* write mode */
     if (*connssl).connecting_state as libc::c_uint
         == ssl_connect_2_writing as libc::c_uint
     {/* read mode */
         *socks
             .offset(0 as isize) = (*conn).sock[0 as usize];
         return (1 as i32) << 16 as i32 + 0 as i32;
     }
     if (*connssl).connecting_state as libc::c_uint
         == ssl_connect_2_reading as libc::c_uint
     {
         *socks
             .offset(0 as isize) = (*conn).sock[0 as usize];
 ...
 pub extern "C" fn Curl_ssl_close(
     ...
     unsafe{
     #[cfg(all(DEBUGBUILD, HAVE_ASSERT_H))]
     if sockindex <= 1 as i32 && sockindex >= -(1 as i32) {} else {
         __assert_fail(
 ...
 */
     ...
     if !((*data).state.session).is_null() {/* this is just a precaution to prevent multiple inits */
         return CURLE_OK;
     }
     match () {
         #[cfg(not(CURLDEBUG))]
 ...
 #[cfg(USE_SSL)]
 #[no_mangle]
 pub extern "C" fn Curl_ssl_version(
 ...
 */
     ...
     memcpy(
         &mut *output
             .offset(labellen.wrapping_add(1 as u64) as isize)
 ...
 */
     ...
     pem_count = begin_pos.offset_from(pem) as size_t;
      /* Invalid if not at beginning AND not directly following \n */
     if 0 as u64 != pem_count
         && '\n' as i32
             != *pem
                 .offset(
                     pem_count.wrapping_sub(1 as u64) as isize,
                 ) as i32
     {
         return CURLE_BAD_CONTENT_ENCODING;
     }
     ...
     pem_count = (pem_count as u64)
         .wrapping_add(26 as u64) as size_t as size_t;
     ...
     pem_len = end_pos.offset_from(pem) as size_t;
     match () {
         #[cfg(not(CURLDEBUG))]
         _ => {
 ...
 */
     ...
     ) == 0 as i32
     {
         ...
         match () {
             ...
             _ => {
                 ...
                 .expect("non-null function pointer")(32 as size_t)
                 as *mut u8;
             }
             #[cfg(CURLDEBUG)]
             _ => {
         ...
         encode = ((*Curl_ssl).sha256sum)
             .expect(
                 "non-null function pointer",
             )(pubkey, pubkeylen, sha256sumdigest, 32 as size_t);
       ...
       */
                 if !end_pos.is_null() {
                     *end_pos
                         .offset(0 as isize) = ';' as libc::c_char;
                     begin_pos = strstr(
                         end_pos,
                         b"sha256//\0" as *const u8 as *const libc::c_char,
                     );
     ...
     if !(fseek(fp, 0 as i64, 2 as i32) != 0) {
         filesize = ftell(fp);
         if !(fseek(fp, 0 as i64, 0 as i32) != 0) {
             if !(filesize < 0 as i64
                 || filesize > 1048576 as i64)
             {
                /*
 ...
 */
 ...
 extern "C" fn multissl_version(
     ...
     if current != selected {
         ...
         let mut end: *mut libc::c_char = backends
             .as_mut_ptr()
             .offset(
                 ::std::mem::size_of::<[libc::c_char; 200]>() as isize,
             );
         let mut i: i32 = 0;
         selected = current;
     ...
     if size <= backends_len {
         strncpy(
             buffer,
             backends.as_mut_ptr(),
             size.wrapping_sub(1 as u64),
         );
         *buffer
             .offset(
                 size.wrapping_sub(1 as u64) as isize,
             ) = '\0' as libc::c_char;
 ...
 extern "C" fn multissl_setup(mut backend: *const Curl_ssl) -> i32 {
     ...
     if (available_backends[0 as usize]).is_null() {
         return 1 as i32;
     }
 ...
 pub extern "C" fn curl_global_sslset(
     ...
     if Curl_ssl != &Curl_ssl_multi as *const Curl_ssl {
         return (if id as u32== (*Curl_ssl).info.id as libc::c_uint
