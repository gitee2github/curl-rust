...
extern "C" fn wolfssl_connect_step1(
    ...
    unsafe {
        ...
        if !(SSL_SET_OPTION_primary_clientcert).is_null() && !(SSL_SET_OPTION_key).is_null() {
            ...
            if wolfSSL_CTX_use_PrivateKey_file((*backend).ctx, SSL_SET_OPTION_key, file_type)
                != 1 as i32
            {
                Curl_failf(
                    data,
...
extern "C" fn wolfssl_connect_step2(
    ...
    unsafe {
        ...
        if ret != 1 as i32 {
            ...
            let mut detail: i32 = wolfSSL_get_error((*backend).wolf_handle, ret);
    ...
    unsafe {
        ...
        if SSL_SET_OPTION_primary_sessionid != 0 {
            ...
            let mut our_ssl_sessionid: *mut SSL_SESSION =
                wolfSSL_get_session((*backend).wolf_handle);
            #[cfg(not(CURL_DISABLE_PROXY))]
            let mut isproxy: bool = if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
                && ssl_connection_complete as u32
                    != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                        0 as i32
                    } else {
...
extern "C" fn wolfssl_send(
    ...
    unsafe {
        ...
        if rc <= 0 as i32 {
            let mut err: i32 = wolfSSL_get_error((*backend).wolf_handle, rc);
            match err {
                2 | 3 => {
...
extern "C" fn wolfssl_recv(
    ...
    unsafe {
        ...
        if nread < 0 as i32 {
            let mut err: i32 = wolfSSL_get_error((*backend).wolf_handle, nread);
            match err {
                6 => {}
                2 | 3 => {
...
extern "C" fn wolfssl_data_pending(mut conn: *const connectdata, mut connindex: i32) -> bool {
    unsafe {
        ...
        if !((*backend).wolf_handle).is_null() {
            /* SSL is in use */
            return if 0 as i32 != wolfSSL_pending((*backend).wolf_handle) {
                1 as i32
            } else {
                0 as i32
            } != 0;
