...
#[inline]
extern "C" fn stat(mut __path: *const libc::c_char, mut __statbuf: *mut stat) -> i32 {
    unsafe {
...
extern "C" fn nss_error_to_name(mut code: PRErrorCode) -> *const libc::c_char {
    let mut name: *const libc::c_char = unsafe { PR_ErrorToName(code) };
    if !name.is_null() {
        return name;
    }
...
extern "C" fn set_ciphers(
    ...
    let num_implemented_ciphers: PRUint16 = unsafe { SSL_GetNumImplementedCiphers() };
    let mut implemented_ciphers: *const PRUint16 = unsafe { SSL_GetImplementedCiphers() };
    ...
    cipher = cipher_list;
    while !cipher_list.is_null() && unsafe { *cipher_list.offset(0 as isize) as i32 } != 0 {
        while unsafe { *cipher as i32 != 0 && Curl_isspace(*cipher as i32) != 0 } {
            cipher = unsafe { cipher.offset(1) };
        }
        cipher_list = unsafe { strpbrk(cipher, b":, \0" as *const u8 as *const libc::c_char) };
        ...
        while (i as u64)
            < (::std::mem::size_of::<[cipher_s; 94]>() as u64)
                .wrapping_div(::std::mem::size_of::<cipher_s>() as u64)
        {
            if unsafe { Curl_strcasecompare(cipher, cipherlist[i as usize].name) } != 0 {
            ...
            .wrapping_div(::std::mem::size_of::<cipher_s>() as u64)
    {
        if !(cipher_state[i as usize] == 0) {
            if unsafe {
                SSL_CipherPrefSet(model, cipherlist[i as usize].num, 1 as i32) as i32
                    != SECSuccess as i32
            } {
                unsafe {
                    Curl_failf(
                        data,
                        b"cipher-suite not supported by NSS: %s\0" as *const u8
...
extern "C" fn is_file(mut filename: *const libc::c_char) -> i32 {
    ...
    if stat(filename, &mut st) == 0 as i32 {
        if st.st_mode & 0o170000 as u32 == 0o100000 as u32
            || st.st_mode & 0o170000 as u32 == 0o10000 as u32
            || st.st_mode & 0o170000 as u32 == 0o20000 as u32
        {
            return 1 as i32;
        }
    }
 ...
 */
extern "C" fn nss_find_slot_by_name(mut slot_name: *const libc::c_char) -> *mut PK11SlotInfo {
...
extern "C" fn insert_wrapped_ptr(
    ...
    #[cfg(not(CURLDEBUG))]
    let mut wrap: *mut ptr_list_wrap = unsafe {
...
extern "C" fn nss_create_object(
    ...
    let mut slot_name: *mut libc::c_char = unsafe {
        curl_maprintf(
    ...
    attr_cnt = attr_cnt + 1;
    let mut ptr: *mut CK_ATTRIBUTE = unsafe { attrs.as_mut_ptr().offset(attr_cnt as isize) };
    unsafe {
    ...
    attr_cnt = attr_cnt + 1;
    let mut ptr_0: *mut CK_ATTRIBUTE = unsafe { attrs.as_mut_ptr().offset(attr_cnt as isize) };
    unsafe {
    ...
    attr_cnt = attr_cnt + 1;
    let mut ptr_1: *mut CK_ATTRIBUTE = unsafe { attrs.as_mut_ptr().offset(attr_cnt as isize) };
    unsafe {
    ...
    if 0x1 as u64 == obj_class {
        ...
        attr_cnt = attr_cnt + 1;
        let mut ptr_2: *mut CK_ATTRIBUTE = unsafe { attrs.as_mut_ptr().offset(attr_cnt as isize) };
        unsafe {
            (*ptr_2).type_0 = (0x80000000 as u32 | 0x4e534350 as u32).wrapping_add(0x2000 as u32)
    ...
    if cfg!(HAVE_PK11_CREATEMANAGEDGENERICOBJECT) {
        obj = unsafe {
            PK11_CreateManagedGenericObject(slot, attrs.as_mut_ptr(), attr_cnt, 0 as i32)
        };
    }
    unsafe {
        PK11_FreeSlot(slot);
    ...
    if unsafe { insert_wrapped_ptr(&mut (*backend).obj_list, obj as *mut libc::c_void) as u32 }
        != CURLE_OK as u32
    {
        unsafe {
...
extern "C" fn nss_destroy_object(mut user: *mut libc::c_void, mut ptr: *mut libc::c_void) {
    ...
    let mut obj: *mut PK11GenericObject = unsafe { (*wrap).ptr as *mut PK11GenericObject };
    unsafe {
        PK11_DestroyGenericObject(obj);
    }
    #[cfg(not(CURLDEBUG))]
...
extern "C" fn nss_destroy_crl_item(mut user: *mut libc::c_void, mut ptr: *mut libc::c_void) {
    let mut wrap: *mut ptr_list_wrap = ptr as *mut ptr_list_wrap;
    let mut crl_der: *mut SECItem = unsafe { (*wrap).ptr as *mut SECItem };
    unsafe {
...
extern "C" fn nss_load_cert(
    ...
    if is_file(filename) != 0 {
        ...
        result = nss_create_object(ssl, 0x1 as CK_OBJECT_CLASS, filename, cacert != 0);
    }
    if result as u64 == 0 && cacert == 0 {
        let mut nickname: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut n: *mut libc::c_char = unsafe { strrchr(filename, '/' as i32) };
        if !n.is_null() {
            n = unsafe { n.offset(1) };
        }
        ...
        nickname =
            unsafe { curl_maprintf(b"PEM Token #1:%s\0" as *const u8 as *const libc::c_char, n) };
        if !nickname.is_null() {
            let mut cert: *mut CERTCertificate =
                unsafe { PK11_FindCertFromNickname(nickname, 0 as *mut libc::c_void) };
            if !cert.is_null() {
...
extern "C" fn nss_cache_crl(mut crl_der: *mut SECItem) -> CURLcode {
    let mut db: *mut CERTCertDBHandle = unsafe { CERT_GetDefaultCertDB() };
    let mut crl: *mut CERTSignedCrl = unsafe { SEC_FindCrlByDERCert(db, crl_der, 0 as i32) };
    if !crl.is_null() {
        /* CRL already cached */
        unsafe {
            SEC_DestroyCrl(crl);
    ...
    if SECSuccess as i32 != unsafe { CERT_CacheCRL(db, crl_der) as i32 } {
        /* unable to cache CRL */
        unsafe {
            SECITEM_FreeItem(crl_der, 1 as i32);
    ...
    if unsafe {
        insert_wrapped_ptr(&mut nss_crl_list, crl_der as *mut libc::c_void) as u32
            != CURLE_OK as u32
    } {
        if SECSuccess as i32 == unsafe { CERT_UncacheCRL(db, crl_der) as i32 } {
            unsafe {
                SECITEM_FreeItem(crl_der, 1 as i32);
            }
        }
...
extern "C" fn nss_load_crl(mut crlfilename: *const libc::c_char) -> CURLcode {
    ...
    let mut crl_der: *mut SECItem = 0 as *mut SECItem;
    let mut body: *mut libc::c_char = 0 as *mut libc::c_char;
    infile = unsafe { PR_Open(crlfilename, 0x1 as i32, 0 as i32) };
    if infile.is_null() {
        return CURLE_SSL_CRL_BADFILE;
    }
    // 创建一个循环
    'fail: loop {
        ...
        if info.size != unsafe { PR_Read(infile, filedata.data as *mut libc::c_void, info.size) } {
            break 'fail;
        }
        crl_der = unsafe { SECITEM_AllocItem(0 as *mut PLArenaPool, 0 as *mut SECItem, 0 as u32) };
        if crl_der.is_null() {
            break 'fail;
        }
        ...
        if !body.is_null() {
            /* assume ASCII */
            let mut trailer: *mut libc::c_char = 0 as *mut libc::c_char;
            let mut begin: *mut libc::c_char = unsafe { strchr(body, '\n' as i32) };
            if begin.is_null() {
                begin = unsafe { strchr(body, '\r' as i32) };
            }
            if begin.is_null() {
                break 'fail;
            }
            begin = unsafe { begin.offset(1) };
            trailer = unsafe { strstr(begin, b"-----END\0" as *const u8 as *const libc::c_char) };
            if trailer.is_null() {
                break 'fail;
            }
...
extern "C" fn nss_load_key(
    ...
    let mut ssl: *mut ssl_connect_data = unsafe { ((*conn).ssl).as_mut_ptr() };
    result = nss_create_object(ssl, 0x3 as CK_OBJECT_CLASS, key_file, 0 as i32 != 0);
    if result as u64 != 0 {
        unsafe {
    ...
    tmp = unsafe { SECMOD_WaitForAnyTokenEvent(pem_module, 0 as u64, 0 as PRIntervalTime) };
    if !tmp.is_null() {
        unsafe {
            PK11_FreeSlot(tmp);
        }
    }
    if unsafe { PK11_IsPresent(slot) == 0 } {
        unsafe {
            PK11_FreeSlot(slot);
        }
        return CURLE_SSL_CERTPROBLEM;
    ...
    let SSL_SET_OPTION_key_passwd = unsafe {
        PK11_Authenticate(
            slot,
            1 as i32,
...
extern "C" fn cert_stuff(
    ...
    if !key_file.is_null() || is_file(cert_file) != 0 {
        ...
        if result as u64 != 0 {
            let err_0: PRErrorCode = unsafe { PR_GetError() };
...
extern "C" fn nss_get_password(
    ...
    if retry != 0 || arg.is_null() {
        ...
        return unsafe { PORT_Strdup(arg as *mut libc::c_char) };
    };
}
...
extern "C" fn nss_auth_cert_hook(
    ...
    #[cfg(not(CURL_DISABLE_PROXY))]
    let SSL_CONN_CONFIG_verifystatus = unsafe {
    ...
    #[cfg(CURL_DISABLE_PROXY)]
    let SSL_CONN_CONFIG_verifystatus = unsafe { ((*conn).ssl_config).verifystatus() };
    #[cfg(not(CURL_DISABLE_PROXY))]
    let SSL_CONN_CONFIG_verifypeer = unsafe {
        if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
            && ssl_connection_complete as u32
                != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                    0 as i32
    ...
    if SSL_CONN_CONFIG_verifystatus != 0 {
        ...
        let mut csa: *const SECItemArray = unsafe { SSL_PeerStapledOCSPResponses(fd) };
        if csa.is_null() {
            unsafe {
                Curl_failf(
                    data,
...
extern "C" fn HandshakeCallback(mut sock: *mut PRFileDesc, mut arg: *mut libc::c_void) {
    ...
    if unsafe { ((*conn).bits).tls_enable_npn() == 0 && ((*conn).bits).tls_enable_alpn() == 0 } {
        return;
    }
    if unsafe {
        SSL_GetNextProto(sock, &mut state, buf.as_mut_ptr(), &mut buflen, buflenmax) as i32
    } == SECSuccess as i32
    {
...
extern "C" fn CanFalseStartCallback(
    ...
    'end: loop {
        ...
        if cipherInfo.symCipher as u32 != ssl_calg_aes_gcm as u32 {
            break 'end;
        }
...
extern "C" fn display_cert_info(mut data: *mut Curl_easy, mut cert: *mut CERTCertificate) {
    ...
    subject = unsafe { CERT_NameToAscii(&mut (*cert).subject) };
    issuer = unsafe { CERT_NameToAscii(&mut (*cert).issuer) };
    common_name = unsafe { CERT_GetCommonName(&mut (*cert).subject) };
    unsafe {
        Curl_infof(
            data,
        ...
        PR_ExplodeTime(
            notBefore,
            Some(
                PR_GMTParameters as unsafe extern "C" fn(*const PRExplodedTime) -> PRTimeParameters,
            ),
            &mut printableTime,
        );
        PR_FormatTime(
...
extern "C" fn display_conn_info(mut data: *mut Curl_easy, mut sock: *mut PRFileDesc) -> CURLcode {
    let mut result: CURLcode = CURLE_OK;
    ...
    if unsafe {
        ...
        == SECSuccess as i32
        && channel.length as u64 == ::std::mem::size_of::<SSLChannelInfo>() as u64
    ...
    cert = unsafe { SSL_PeerCertificate(sock) };
    if !cert.is_null() {
        unsafe {
            Curl_infof(
                data,
        ...
        if unsafe { ((*data).set.ssl).certinfo() == 0 } {
            ...
            now = unsafe { PR_Now() };
            if unsafe { (*cert).isRoot == 0 } {
                cert2 = unsafe { CERT_FindCertIssuer(cert, now, certUsageSSLCA) };
                while !cert2.is_null() {
                    i += 1;
                    if unsafe { (*cert2).isRoot != 0 } {
                        unsafe {
                            CERT_DestroyCertificate(cert2);
                        }
                        break;
                    } else {
                        cert3 = unsafe { CERT_FindCertIssuer(cert2, now, certUsageSSLCA) };
                        unsafe {
                            CERT_DestroyCertificate(cert2);
                        }
                        cert2 = cert3;
...
#[cfg(not(CURL_DISABLE_PROXY))]
extern "C" fn BadCertHandler(mut arg: *mut libc::c_void, mut sock: *mut PRFileDesc) -> SECStatus {
    let mut data: *mut Curl_easy = arg as *mut Curl_easy;
    let mut conn: *mut connectdata = unsafe { (*data).conn };
    let mut err: PRErrorCode = unsafe { PR_GetError() };
    let mut cert: *mut CERTCertificate = 0 as *mut CERTCertificate;
...
extern "C" fn BadCertHandler(mut arg: *mut libc::c_void, mut sock: *mut PRFileDesc) -> SECStatus {
    ...
    if err == SSL_ERROR_BAD_CERT_DOMAIN as i32 && unsafe { ((*conn).ssl_config).verifyhost() == 0 }
    {
...
extern "C" fn check_issuer_cert(
    ...
    if cert_issuer.is_null() || issuer.is_null() {
        ...
        SECITEM_CompareItem(&mut (*cert_issuer).derCert, &mut (*issuer).derCert) as i32
    } != SECEqual as i32
    {
        res = SECFailure;
    }
...
extern "C" fn cmp_peer_pubkey(
    ...
    let mut backend: *mut ssl_backend_data = unsafe { (*connssl).backend };
    let mut data: *mut Curl_easy = unsafe { (*backend).data };
    let mut cert: *mut CERTCertificate = 0 as *mut CERTCertificate;
    if pinnedpubkey.is_null() {
        return CURLE_OK;
    }
    cert = unsafe { SSL_PeerCertificate((*backend).nss_handle) };
    if !cert.is_null() {
        /* no pinned public key specified */
        let mut pubkey: *mut SECKEYPublicKey = unsafe { CERT_ExtractPublicKey(cert) };
        if !pubkey.is_null() {
            let mut cert_der: *mut SECItem = unsafe { PK11_DEREncodePublicKey(pubkey) };
            if !cert_der.is_null() {
                result = unsafe {
...
extern "C" fn SelectClientCert(
    ...
    let mut backend: *mut ssl_backend_data = unsafe { (*connssl).backend };
    let mut data: *mut Curl_easy = unsafe { (*backend).data };
    let mut nickname: *const libc::c_char = unsafe { (*backend).client_nickname };
    static mut pem_slotname: [libc::c_char; 13] =
        unsafe { *::std::mem::transmute::<&[u8; 13], &[libc::c_char; 13]>(b"PEM Token #1\0") };
    if unsafe { !((*backend).obj_clicert).is_null() } {
        ...
        let mut proto_win: *mut libc::c_void = unsafe { SSL_RevealPinArg(sock) };
        ...
        let mut slot: *mut PK11SlotInfo = unsafe { nss_find_slot_by_name(pem_slotname.as_ptr()) };
        if slot.is_null() {
            unsafe {
                Curl_failf(
                    data,
        ...
        cert = unsafe { PK11_FindCertFromDERCertItem(slot, &mut cert_der, proto_win) };
        unsafe { SECITEM_FreeItem(&mut cert_der, 0 as i32) };
        if cert.is_null() {
        ...
        key = unsafe { PK11_FindPrivateKeyFromCert(slot, cert, 0 as *mut libc::c_void) };
        unsafe {
            PK11_FreeSlot(slot);
        }
        if key.is_null() {
    ...
    nickname = unsafe { (**pRetCert).nickname };
    if nickname.is_null() {
    ...
    if unsafe {
        strncmp(
            nickname,
            pem_slotname.as_ptr(),
            (::std::mem::size_of::<[libc::c_char; 13]>() as u64).wrapping_sub(1 as u64),
        ) == 0
    } {
        unsafe {
            Curl_failf(
    ...
    if unsafe { (*pRetKey).is_null() } {
        unsafe {
            Curl_failf(
                data,
                b"NSS: private key not found for certificate: %s\0" as *const u8
                    as *const libc::c_char,
                nickname,
            );
        }
...
extern "C" fn nss_update_connecting_state(
    ...
    match unsafe { (*connssl).connecting_state as u32 } {
        1 | 2 | 3 => {}
        _ => return,
    }
...
extern "C" fn nspr_io_recv(
    ...
    let recv_fn: PRRecvFN = unsafe { (*(*(*fd).lower).methods).recv };
    let rv: PRInt32 = unsafe {
...
extern "C" fn nspr_io_send(
    ...
    let send_fn: PRSendFN = unsafe { (*(*(*fd).lower).methods).send };
    let rv: PRInt32 = unsafe {
...
extern "C" fn nspr_io_close(mut fd: *mut PRFileDesc) -> PRStatus {
    let close_fn: PRCloseFN = unsafe { (*PR_GetDefaultIOMethods()).close };
    unsafe {
        (*fd).secret = 0 as *mut PRFilePrivate;
    }
    return unsafe { close_fn.expect("non-null function pointer")(fd) };
}
...
extern "C" fn nss_load_module(
    ...
    let mut module: *mut SECMODModule = unsafe { *pmod };
    if !module.is_null() {
        /* already loaded */
        return CURLE_OK;
    }
    config_string = unsafe {
        curl_maprintf(
    ...
    module = unsafe { SECMOD_LoadUserModule(config_string, 0 as *mut SECMODModule, 0 as i32) };
    #[cfg(not(CURLDEBUG))]
    unsafe {
    ...
    if !module.is_null() && unsafe { (*module).loaded != 0 } {
        /* loaded successfully */
        unsafe {
            *pmod = module;
        }
...
extern "C" fn nss_unload_module(mut pmod: *mut *mut SECMODModule) {
    ...
    if unsafe { SECMOD_UnloadUserModule(module) as i32 != SECSuccess as i32 } {
        /* unload failed */
        return;
    }
    unsafe {
...
extern "C" fn nss_init_core(
    ...
    if !cert_dir.is_null() {
        let mut certpath: *mut libc::c_char =
            unsafe { curl_maprintf(b"sql:%s\0" as *const u8 as *const libc::c_char, cert_dir) };
        if certpath.is_null() {
            return CURLE_OUT_OF_MEMORY;
        }
        unsafe {
...
extern "C" fn nss_setup(mut data: *mut Curl_easy) -> CURLcode {
    ...
    cert_dir = unsafe { getenv(b"SSL_DIR\0" as *const u8 as *const libc::c_char) };
    if !cert_dir.is_null() {
        if stat(cert_dir, &mut st) != 0 as i32 || !(st.st_mode & 0o170000 as u32 == 0o40000 as u32)
        {
    ...
    if cert_dir.is_null() {
        ...
        if stat(
            ...
            && st.st_mode & 0o170000 as u32 == 0o40000 as u32
        {
            cert_dir = b"/etc/pki/nssdb\0" as *const u8 as *const libc::c_char as *mut libc::c_char;
        }
    }
    if unsafe { nspr_io_identity == -(1 as i32) } {
        unsafe {
            nspr_io_identity =
                PR_GetUniqueIdentity(b"libcurl\0" as *const u8 as *const libc::c_char);
        }
        if unsafe { nspr_io_identity == -(1 as i32) } {
            return CURLE_OUT_OF_MEMORY;
        }
...
extern "C" fn nss_init() -> i32 {
    ...
    if unsafe { nss_initlock.is_null() } {
        unsafe {
            PR_Init(PR_USER_THREAD, PR_PRIORITY_NORMAL, 0 as PRUintn);
...
extern "C" fn close_one(mut connssl: *mut ssl_connect_data) {
    ...
    let client_cert: bool =
        unsafe { !((*backend).client_nickname).is_null() || !((*backend).obj_clicert).is_null() };
    if unsafe { !((*backend).nss_handle).is_null() } {
 ...
 */
extern "C" fn nss_close(mut data: *mut Curl_easy, mut conn: *mut connectdata, mut sockindex: i32) {
    let mut connssl: *mut ssl_connect_data = unsafe {
        &mut *((*conn).ssl).as_mut_ptr().offset(sockindex as isize) as *mut ssl_connect_data
    };
    #[cfg(not(CURL_DISABLE_PROXY))]
    let mut connssl_proxy: *mut ssl_connect_data = unsafe {
        &mut *((*conn).proxy_ssl).as_mut_ptr().offset(sockindex as isize) as *mut ssl_connect_data
    };

    let mut backend: *mut ssl_backend_data = unsafe { (*connssl).backend };
    #[cfg(not(CURL_DISABLE_PROXY))]
    let CURL_DISABLE_PROXY_3 = unsafe { !((*(*connssl_proxy).backend).nss_handle).is_null() };
    #[cfg(CURL_DISABLE_PROXY)]
    let CURL_DISABLE_PROXY_3 = false;
    if unsafe { !((*backend).nss_handle).is_null() } || CURL_DISABLE_PROXY_3 {
        unsafe {
            #[cfg(CURLDEBUG)]
...
extern "C" fn nss_load_ca_certificates(
    ...
    #[cfg(not(CURL_DISABLE_PROXY))]
    let mut cafile: *const libc::c_char = unsafe {
        if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
            && ssl_connection_complete as u32
                != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                    0 as i32
    ...
    #[cfg(CURL_DISABLE_PROXY)]
    let mut cafile: *const libc::c_char = unsafe { (*conn).ssl_config.CAfile };
    #[cfg(not(CURL_DISABLE_PROXY))]
    let mut capath: *const libc::c_char = unsafe {
        if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
            && ssl_connection_complete as u32
                != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                    0 as i32
    ...
    #[cfg(CURL_DISABLE_PROXY)]
    let mut capath: *const libc::c_char = unsafe { (*conn).ssl_config.CApath };
    let mut use_trust_module: bool = false;
    let mut result: CURLcode = CURLE_OK;

    /* treat empty string as unset */
    if !cafile.is_null() && unsafe { *cafile.offset(0 as isize) == 0 } {
        cafile = 0 as *const libc::c_char;
    }
    if !capath.is_null() && unsafe { *capath.offset(0 as isize) == 0 } {
    ...
    if use_trust_module as i32 != 0 && unsafe { trust_module.is_null() } {
    ...
    } else if !use_trust_module && !unsafe { trust_module.is_null() } {
    ...
    if !capath.is_null() {
        ...
        if st.st_mode & 0o170000 as u32 == 0o40000 as u32 {
            let mut entry: *mut PRDirEntry = 0 as *mut PRDirEntry;
            let mut dir: *mut PRDir = unsafe { PR_OpenDir(capath) };
            if dir.is_null() {
                return CURLE_SSL_CACERT_BADFILE;
            }
            loop {
                ...
                let mut fullpath: *mut libc::c_char = unsafe {
                    curl_maprintf(
                        b"%s/%s\0" as *const u8 as *const libc::c_char,
                        capath,
                        (*entry).name,
...
extern "C" fn nss_sslver_from_curl(mut nssver: *mut PRUint16, mut version: i64) -> CURLcode {
    match version {
        2 => {
            unsafe {
...
extern "C" fn nss_init_sslver(
    ...
    #[cfg(CURL_DISABLE_PROXY)]
    let min: i64 = unsafe { (*conn).ssl_config.version };
    #[cfg(not(CURL_DISABLE_PROXY))]
    let max: i64 = unsafe {
        if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
            && ssl_connection_complete as u32
    ...
    match min {
        1 | 0 => {
            ...
            if unsafe { SSL_VersionRangeGetDefault(ssl_variant_stream, &mut vrange) as i32 }
                != SECSuccess as i32
            {
                return CURLE_SSL_CONNECT_ERROR;
            }
            if unsafe { ((*sslver).min as i32) < vrange.min as i32 } {
                unsafe {
        ...
        _ => {
            result = unsafe { nss_sslver_from_curl(&mut (*sslver).min, min) };
            if result as u64 != 0 {
                unsafe {
                    Curl_failf(
                        data,
    ...
    match max {
        0 | 65536 => {}
        _ => {
            result = unsafe { nss_sslver_from_curl(&mut (*sslver).max, max >> 16 as i32) };
            if result as u64 != 0 {
                unsafe {
                    Curl_failf(
                        data,
...
extern "C" fn nss_set_blocking(
    ...
    if unsafe {
        PR_SetSocketOption((*backend).nss_handle, &mut sock_opt) as i32 != PR_SUCCESS as i32
    } {
...
extern "C" fn nss_setup_connect(
    ...
    let mut sslver: SSLVersionRange = {
        ...
        #[cfg(all(
            not(SSL_LIBRARY_VERSION_TLS_1_3),
            not(SSL_LIBRARY_VERSION_TLS_1_2),
            not(SSL_LIBRARY_VERSION_TLS_1_1)
        ))]
        let mut init = SSLVersionRangeStr {
            min: 0x301 as PRUint16,
    ...
    'error: loop {
        ...
        result = CURLE_SSL_CONNECT_ERROR;
        model = unsafe { PR_NewTCPSocket() };
        if model.is_null() {
            break 'error;
        }
        model = unsafe { SSL_ImportFD(0 as *mut PRFileDesc, model) };
        if unsafe { SSL_OptionSet(model, 1 as i32, 1 as i32) } as i32 != SECSuccess as i32 {
            break 'error;
        }
        if unsafe { SSL_OptionSet(model, 6 as i32, 0 as i32) } as i32 != SECSuccess as i32 {
            break 'error;
        }
        if unsafe { SSL_OptionSet(model, 5 as i32, 1 as i32) } as i32 != SECSuccess as i32 {
            break 'error;
        }
        #[cfg(not(CURL_DISABLE_PROXY))]
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let ssl_no_cache_value = if unsafe {
            ((*data).set.ssl.primary).sessionid() as i32 != 0
        ...
        if unsafe { SSL_OptionSet(model, 9 as i32, ssl_no_cache) as i32 != SECSuccess as i32 } {
            break 'error;
        }
        /* enable/disable the requested SSL version(s) */
        if nss_init_sslver(&mut sslver, data, conn) as u32 != CURLE_OK as u32 {
            break 'error;
        }
        if unsafe { SSL_VersionRangeGetSupported(ssl_variant_stream, &mut sslver_supported) as i32 }
            != SECSuccess as i32
        {
            break 'error;
        }
        ...
        if unsafe { SSL_VersionRangeSet(model, &mut sslver) as i32 != SECSuccess as i32 } {
            break 'error;
        }
        #[cfg(not(CURL_DISABLE_PROXY))]
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let ssl_cbc_random_iv_value = unsafe { (((*data).set.ssl).enable_beast() == 0) as i32 };
        ssl_cbc_random_iv = ssl_cbc_random_iv_value;
        #[cfg(SSL_CBC_RANDOM_IV)]
        /* unless the user explicitly asks to allow the protocol vulnerability, we
        use the work-around */
        if unsafe { SSL_OptionSet(model, 23 as i32, ssl_cbc_random_iv) as i32 != SECSuccess as i32 }
        {
            unsafe {
                Curl_infof(
                    data,
        ...
        if !SSL_CONN_CONFIG_cipher_list.is_null() {
            if set_ciphers(data, model, SSL_CONN_CONFIG_cipher_list) as i32 != SECSuccess as i32 {
                result = CURLE_SSL_CIPHER;
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let SSL_CONN_CONFIG_verifyhost = unsafe { ((*conn).ssl_config).verifyhost() };

        if (SSL_CONN_CONFIG_verifypeer) == 0 && (SSL_CONN_CONFIG_verifyhost) != 0 {
            unsafe {
                Curl_infof(
                    data,
                    b"warning: ignoring value of ssl.verifyhost\0" as *const u8
        ...
        if true {
            unsafe {
                (*data).set.ssl.certverifyresult = 0 as i64;
            }
        }
        if unsafe {
            SSL_BadCertHook(
        ...
        if unsafe {
            SSL_HandshakeCallback(
                ...
                Some(
                    HandshakeCallback
                        as unsafe extern "C" fn(*mut PRFileDesc, *mut libc::c_void) -> (),
                ),
                data as *mut libc::c_void,
            )
        } as i32
        ...
        if rv as u32 == CURLE_SSL_CACERT_BADFILE as u32 && (SSL_CONN_CONFIG_verifypeer) == 0 {
            unsafe {
                Curl_infof(
                    data,
                    b"warning: CA certificates failed to load\0" as *const u8
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let SSL_SET_OPTION_CRLfile = unsafe { (*data).set.ssl.CRLfile };
        ...
        if !SSL_SET_OPTION_primary_clientcert.is_null() {
            let mut nickname: *mut libc::c_char =
                dup_nickname(data, SSL_SET_OPTION_primary_clientcert);
            if !nickname.is_null() {
                unsafe {
                    (*backend).obj_clicert = 0 as *mut PK11GenericObject;
                }
        ...
        if unsafe { ((*conn).proxy_ssl[sockindex as usize]).use_0() } != 0 {
            ...
            if ssl_connection_complete as libc::c_int as libc::c_uint
                == unsafe { (*conn).proxy_ssl[sockindex as usize].state as libc::c_uint }
            {
            } else {
                unsafe {
                    __assert_fail(
            ...
            #[cfg(all(DEBUGBUILD, HAVE_ASSERT_H))]
            if unsafe { !((*(*conn).proxy_ssl[sockindex as usize].backend).handle).is_null() } {
            } else {
                unsafe {
                    __assert_fail(
            ...
            nspr_io = unsafe { PR_ImportTCPSocket(sockfd) };
            if nspr_io.is_null() {
                break 'error;
            }
        }
        ...
        nspr_io_stub = unsafe { PR_CreateIOLayerStub(nspr_io_identity, &mut nspr_io_methods) };
        if nspr_io_stub.is_null() {
            if !second_layer {
                unsafe {
                    PR_Close(nspr_io);
        ...
        if unsafe { PR_PushIOLayer(nspr_io, -(2 as i32), nspr_io_stub) as i32 != PR_SUCCESS as i32 }
        {
            if !second_layer {
                unsafe {
                    PR_Close(nspr_io);
        ...
        if SSL_CONN_CONFIG_verifystatus != 0 {
            if unsafe {
                SSL_OptionSet((*backend).nss_handle, 24 as i32, 1 as i32) as i32
                    != SECSuccess as i32
            } {
                break 'error;
            }
        ...
        if unsafe { ((*data).set.ssl).falsestart() } != 0 {
            if unsafe { SSL_OptionSet((*backend).nss_handle, 22 as i32, 1 as i32) } as i32
                != SECSuccess as i32
            {
                break 'error;
            }
        ...
        if unsafe {
            ...
            if true {
                #[cfg(not(CURL_DISABLE_PROXY))]
                let CURL_DISABLE_PROXY_1 = unsafe {
                    (!(CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
                        && ssl_connection_complete as u32
                            != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                                0 as i32
                            } else {
            ...
            cur += 8 as i32;
            if unsafe {
                SSL_SetNextProtoNego((*backend).nss_handle, protocols.as_mut_ptr(), cur as u32)
                    as i32
                    != SECSuccess as i32
            } {
                break 'error;
        ...
        if unsafe {
            SSL_ResetHandshake((*backend).nss_handle, 0 as i32) as i32 != SECSuccess as i32
        } {
            break 'error;
        }
        #[cfg(not(CURL_DISABLE_PROXY))]
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let SSL_HOST_NAME_void = unsafe { (*conn).host.name };
        if unsafe {
            SSL_SetURL((*backend).nss_handle, SSL_HOST_NAME_void) as i32 != SECSuccess as i32
        } {
            break 'error;
        }
        if unsafe {
            SSL_SetSockPeerID((*backend).nss_handle, SSL_HOST_NAME_void) as i32 != SECSuccess as i32
        } {
            break 'error;
        }
        return CURLE_OK;
...
extern "C" fn nss_do_connect(
    ...
    'error: loop {
        ...
        timeout = unsafe { PR_MillisecondsToInterval(time_left as PRUint32) };
        #[cfg(not(CURL_DISABLE_PROXY))]
        let SSL_SET_OPTION_certverifyresult = if CURLPROXY_HTTPS as u32
            ...
            unsafe { (*data).set.proxy_ssl.certverifyresult }
        } else {
            unsafe { (*data).set.ssl.certverifyresult }
        };
        #[cfg(CURL_DISABLE_PROXY)]
        let SSL_SET_OPTION_certverifyresult = unsafe { (*data).set.ssl.certverifyresult };
        if unsafe {
            SSL_ForceHandshakeWithTimeout((*backend).nss_handle, timeout) as i32
                != SECSuccess as i32
        } {
            if unsafe { PR_GetError() as i64 } == -(5998 as i64) {
                return CURLE_AGAIN;
        ...
        result = unsafe { display_conn_info(data, (*backend).nss_handle) };
        if result as u64 != 0 {
            break 'error;
        }
        ...
        if !SSL_CONN_CONFIG_issuercert.is_null() {
            let mut ret: SECStatus = SECFailure;
            let mut nickname: *mut libc::c_char = dup_nickname(data, SSL_CONN_CONFIG_issuercert);
            if !nickname.is_null() {
                ret = unsafe { check_issuer_cert((*backend).nss_handle, nickname) };
                unsafe {
                    Curl_cfree.expect("non-null function pointer")(nickname as *mut libc::c_void);
                }
            }
        ...
        let SSL_PINNED_PUB_KEY_void =
            unsafe { (*data).set.str_0[STRING_SSL_PINNEDPUBLICKEY as usize] };
        result = cmp_peer_pubkey(connssl, SSL_PINNED_PUB_KEY_void);
        if result as u64 != 0 {
            break 'error;
        }
        return CURLE_OK;
...
extern "C" fn nss_connect_common(
    ...
    if unsafe { (*connssl).connecting_state as u32 == ssl_connect_1 as u32 } {
...
extern "C" fn nss_send(
    ...
    if rc < 0 as i64 {
        let mut err: PRInt32 = unsafe { PR_GetError() };
...
extern "C" fn nss_random(
    ...
    if SECSuccess as i32 != unsafe { PK11_GenerateRandom(entropy, curlx_uztosi(length)) as i32 } {
        /* signal a failure */
        return CURLE_FAILED_INIT;
    }
    return CURLE_OK;
...
extern "C" fn nss_sha256sum(
    ...
    let mut SHA256pw: *mut PK11Context = unsafe { PK11_CreateDigestContext(SEC_OID_SHA256) };
...
pub static mut Curl_ssl_nss: Curl_ssl = {
    Curl_ssl {
        ...
        random: Some(
            nss_random as unsafe extern "C" fn(*mut Curl_easy, *mut u8, size_t) -> CURLcode,
        ),
        cert_status_request: Some(nss_cert_status_request as unsafe extern "C" fn() -> bool),
        connect_blocking: Some(
            nss_connect as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, i32) -> CURLcode,
        ),
        ...
        close_one: Some(
            nss_close as unsafe extern "C" fn(*mut Curl_easy, *mut connectdata, i32) -> (),
        ),
        ...
        sha256sum: Some(
            nss_sha256sum as unsafe extern "C" fn(*const u8, size_t, *mut u8, size_t) -> CURLcode,
        ),
