...
extern "C" fn entropy_func_mutex(
     ...
     unsafe{Curl_mbedtlsthreadlock_lock_function(1 as libc::c_int);
 ...
 */
 ...
 // done
extern "C" fn mbedtls_version_from_curl(
...
extern "C" fn set_ssl_version_min_max(
     ...
     let mut connssl: *mut ssl_connect_data = unsafe{&mut *((*conn).ssl)
         .as_mut_ptr()
     ...
     let SSL_CONN_CONFIG_version =unsafe{ if CURLPROXY_HTTPS as libc::c_int as libc::c_uint
     ...
     unsafe{mbedtls_ssl_conf_min_version(
         &mut (*backend).config,
         3 as libc::c_int,
         mbedtls_ver_min,
     );
...
extern "C" fn mbed_connect_step1(
     ...
     let ssl_cafile: *const libc::c_char = if CURLPROXY_HTTPS as libc::c_int
         as libc::c_uint == unsafe{(*conn).http_proxy.proxytype as libc::c_uint}
         && ssl_connection_complete as libc::c_int as libc::c_uint
             != unsafe{(*conn)
                 .proxy_ssl[(if (*conn).sock[1 as libc::c_int as usize]
                     == -(1 as libc::c_int)
     ...
     let verifypeer: bool = if CURLPROXY_HTTPS as libc::c_int as libc::c_uint
         ...
         && unsafe{ssl_connection_complete as libc::c_int as libc::c_uint
             != (*conn)
                 .proxy_ssl[(if (*conn).sock[1 as libc::c_int as usize]
                     == -(1 as libc::c_int)
                 {
                 ...
                 .state as libc::c_uint}
     {
        unsafe{((*conn).proxy_ssl_config).verifypeer() as libc::c_int}
     } else {
        unsafe{ ((*conn).ssl_config).verifypeer() as libc::c_int}
     } != 0;
     #[cfg(CURL_DISABLE_PROXY)]
     ...
     */
     ...
     let port: libc::c_long =unsafe{ (*conn).remote_port as libc::c_long};
     ...
     unsafe{mbedtls_x509_crt_init(&mut (*backend).clicert);}
     if !ssl_cert.is_null() {
     ...
     if !ssl_cert_blob.is_null() {
         let mut blob_data: *const libc::c_uchar = unsafe{(*ssl_cert_blob).data
             as *const libc::c_uchar};
         ret = unsafe{mbedtls_x509_crt_parse(
             &mut (*backend).clicert,
             blob_data,
             (*ssl_cert_blob).len,
         )};
     ...
     unsafe{mbedtls_pk_init(&mut (*backend).pk);}
     // 377 - if(SSL_SET_OPTION(key) || SSL_SET_OPTION(key_blob)) {
        ...
        if !(SSL_SET_OPTION_key).is_null() || !(SSL_SET_OPTION_key_blob).is_null(){ // 377 - if(SSL_SET_OPTION(key) || SSL_SET_OPTION(key_blob)) {
         ...
         { // 378 - if(SSL_SET_OPTION(key)) {
             ret = unsafe{mbedtls_pk_parse_keyfile(
                 &mut (*backend).pk,
                 SSL_SET_OPTION_key,
             ...
             let mut key_data: *const libc::c_uchar = unsafe{(*ssl_key_blob).data
                 as *const libc::c_uchar};
 ...
 // #if MBEDTLS_VERSION_NUMBER >= 0x03000000
             ...
             ret =unsafe{ mbedtls_pk_parse_key(
                 &mut (*backend).pk,
                 key_data,
                 (*ssl_key_blob).len,
         ...
         if ret == 0 as libc::c_int
             && unsafe{!(mbedtls_pk_can_do(&mut (*backend).pk, MBEDTLS_PK_RSA) != 0
     ...
     unsafe{mbedtls_x509_crl_init(&mut (*backend).crl);}
     ...
     ret = unsafe{mbedtls_ssl_config_defaults(
         &mut (*backend).config,
         0 as libc::c_int,
         0 as libc::c_int,
         0 as libc::c_int,
     ...
     unsafe{mbedtls_ssl_conf_cert_profile(&mut (*backend).config, &mbedtls_x509_crt_profile_fr);}
     unsafe{
 ...
 // 471        
    ...
    unsafe{mbedtls_ssl_conf_authmode(&mut (*backend).config, 1 as libc::c_int);}
    unsafe{mbedtls_ssl_conf_rng(
         &mut (*backend).config,
         Some(
             mbedtls_ctr_drbg_random
                 as unsafe extern "C" fn(
     ...
     unsafe{mbedtls_ssl_conf_ciphersuites(
         &mut (*backend).config,
     ...
     let SSL_SET_OPTION_primary_sessionid = if CURLPROXY_HTTPS as libc::c_int as libc::c_uint
         ...
         && unsafe{ssl_connection_complete as libc::c_int as libc::c_uint
             != (*conn)
                 ...
                 .state as libc::c_uint}
     {
        unsafe{((*data).set.proxy_ssl.primary).sessionid() as libc::c_int}
     } else {
        unsafe{ ((*data).set.ssl.primary).sessionid() as libc::c_int}
                                         };
     #[cfg(CURL_DISABLE_PROXY)]
     ...
     if SSL_SET_OPTION_primary_sessionid != 0{
         ...
         #[cfg(CURL_DISABLE_PROXY)]
         let SSL_IS_PROXY_null = unsafe{if 0 as libc::c_int != 0 { 1 as libc::c_int } else { 0 as libc::c_int }} != 0 ;
         if !Curl_ssl_getsessionid(
             ...
             ret = unsafe{mbedtls_ssl_set_session(
                 &mut (*backend).ssl,
     ...
     if !(SSL_SET_OPTION_key).is_null() || !(SSL_SET_OPTION_key_blob).is_null(){
        unsafe{mbedtls_ssl_conf_own_cert(
             &mut (*backend).config,
             &mut (*backend).clicert,
             &mut (*backend).pk,
         );}
...
extern "C" fn mbed_connect_step2(
     ...
     let pinnedpubkey: *const libc::c_char = if CURLPROXY_HTTPS as libc::c_int
         ...
         && unsafe{ssl_connection_complete as libc::c_int as libc::c_uint
             != (*conn)
                 ...
                 .state as libc::c_uint}
     {
        unsafe{(*data).set.str_0[STRING_SSL_PINNEDPUBLICKEY_PROXY as libc::c_int as usize]}
     } else {
        unsafe{ (*data).set.str_0[STRING_SSL_PINNEDPUBLICKEY as libc::c_int as usize]}
     };
     #[cfg(CURL_DISABLE_PROXY)]
     ...
     *fresh3 = Some(mbed_send as Curl_send);
     ...
     ret = unsafe{mbedtls_ssl_get_verify_result(&mut (*backend).ssl) as libc::c_int};
     #[cfg(not(CURL_DISABLE_PROXY))]
     if if CURLPROXY_HTTPS as libc::c_int as libc::c_uint
         ...
         && unsafe{ssl_connection_complete as libc::c_int as libc::c_uint
             != (*conn)
                 ...
                 .state as libc::c_uint}
     {
        unsafe{ ((*conn).proxy_ssl_config).verifyhost() as libc::c_int}
     } else {
        unsafe{((*conn).ssl_config).verifyhost() as libc::c_int}
     } == 0
     {
     ...
     #[cfg(CURL_DISABLE_PROXY)]
     let SSL_CONN_CONFIG_verifyhost_1 =unsafe{ ((*conn).ssl_config).verifypeer() as libc::c_int != 0};
     ...
     if !peercert.is_null() && unsafe{((*data).set).verbose() as libc::c_int} != 0 {
         let bufsize: size_t = 16384 as libc::c_int as size_t;
         #[cfg(not(CURLDEBUG))]
         let mut buffer: *mut libc::c_char = unsafe{Curl_cmalloc
         .expect("non-null function pointer")(bufsize) as *mut libc::c_char};
     #[cfg(CURLDEBUG)]
         let mut buffer: *mut libc::c_char = unsafe{curl_dbg_malloc(
             bufsize,
             655 as libc::c_int,
         ...
         #[cfg(not(CURLDEBUG))]
         unsafe{Curl_cfree.expect("non-null function pointer")(buffer as *mut libc::c_void);}
     ...
     if !pinnedpubkey.is_null() {
         ...
         if unsafe{peercert.is_null() || ((*peercert).raw.p).is_null()
             || (*peercert).raw.len == 0}
         {
            unsafe{Curl_failf(
                 data,
     ...
     unsafe{
         ...
         if pubkey.is_null() {
            ...
            unsafe{mbedtls_x509_crt_init(p);}
 // *************************************************************************
 // 701 - TODO
 // #if MBEDTLS_VERSION_NUMBER >= 0x03000000
             if cfg!(MBEDTLS_VERSION_NUMBER_GT_0X03000000){
             ...
             if unsafe{mbedtls_x509_crt_parse_der(p, (*peercert).raw.p, (*peercert).raw.len) != 0}
             {
                unsafe{Curl_failf(
                     data,
                 ...
                 size = unsafe{mbedtls_pk_write_pubkey_der(
                     &mut (*p).pk,
                     pubkey,
                     (if 38 as libc::c_int + 2 as libc::c_int * 1024 as libc::c_int
                         > 30 as libc::c_int
 ...
 // *************************************************************************                      
         ...
         unsafe{mbedtls_x509_crt_free(p);}
         #[cfg(not(CURLDEBUG))]
     ...
     unsafe{curl_dbg_free(
         ...
         #[cfg(not(CURLDEBUG))]
         unsafe{Curl_cfree.expect("non-null function pointer")(pubkey as *mut libc::c_void);}
    ...
    unsafe{ Curl_infof(data, b"SSL connected\0" as *const u8 as *const libc::c_char);}
     return CURLE_OK;
 }
...
extern "C" fn mbed_connect_step3(
     ...
     if SSL_SET_OPTION_primary_sessionid != 0{
         ...
         unsafe{mbedtls_ssl_session_init(our_ssl_sessionid);}
         ret = unsafe{mbedtls_ssl_get_session(&mut (*backend).ssl, our_ssl_sessionid)};
         if ret != 0 {
             if ret != -(0x7f00 as libc::c_int) {
                unsafe{mbedtls_ssl_session_free(our_ssl_sessionid);}
             }
             #[cfg(not(CURLDEBUG))]
             unsafe{Curl_cfree
             .expect(
 ...
 // 内部没有宏
  extern "C" fn mbed_send(
     ...
     let mut backend: *mut ssl_backend_data = unsafe{ (*connssl).backend};
     let mut ret: libc::c_int = -(1 as libc::c_int);
     ret = unsafe{ mbedtls_ssl_write(&mut (*backend).ssl, mem as *mut libc::c_uchar, len)};
     if ret < 0 as libc::c_int {
        unsafe{ *curlcode = (if ret == -(0x6880 as libc::c_int) {
...
extern "C" fn mbedtls_close(
     ...
     // done - 869
     #[cfg(THREADING_SUPPORT)]
     unsafe{ mbedtls_entropy_free(&mut (*backend).entropy);}
 }
...
extern "C" fn mbedtls_session_free(mut ptr: *mut libc::c_void) {
    unsafe{ mbedtls_ssl_session_free(ptr as *mut mbedtls_ssl_session);}
     #[cfg(not(CURLDEBUG))]
 ...
 // done
extern "C" fn mbedtls_version(
     mut buffer: *mut libc::c_char,
     mut size: size_t,
 ) -> size_t {
     // 909 - done
     if cfg!(MBEDTLS_VERSION_C){
     let mut version: libc::c_uint =unsafe{  mbedtls_version_get_number()};
...
extern "C" fn mbed_connect_common(
     ...
     if unsafe{ ssl_connection_complete as libc::c_int as libc::c_uint
         == (*connssl).state as libc::c_uint}
     {
        unsafe{ *done = 1 as libc::c_int != 0;}
         return CURLE_OK;
     }
     if ssl_connect_1 as libc::c_int as libc::c_uint
         == unsafe{ (*connssl).connecting_state as libc::c_uint}
     {
         timeout_ms = unsafe{ Curl_timeleft(data, 0 as *mut curltime, 1 as libc::c_int != 0)};
             ...
             == unsafe{ (*connssl).connecting_state as libc::c_uint}
     {
         ...
         if unsafe{ (*connssl).connecting_state as libc::c_uint}
             == ssl_connect_2_reading as libc::c_int as libc::c_uint
             || unsafe{ (*connssl).connecting_state as libc::c_uint}
                 == ssl_connect_2_writing as libc::c_int as libc::c_uint
         {
             let mut writefd: curl_socket_t = if ssl_connect_2_writing as libc::c_int
                 as libc::c_uint == unsafe{ (*connssl).connecting_state as libc::c_uint}
             {
                 sockfd
             } else {
                 -(1 as libc::c_int)
         ...
         if unsafe{ retcode as libc::c_uint != 0
 ...
 extern "C" fn mbedtls_connect_nonblocking(
    ...
    unsafe{ return mbed_connect_common(data, conn, sockindex, 1 as libc::c_int != 0, done);}
 }
...
extern "C" fn mbedtls_connect(
     ...
     retcode = unsafe{ mbed_connect_common(
         data,
         conn,
         sockindex,
         0 as libc::c_int != 0,
 ...
 // 内部没有宏
 extern "C" fn mbedtls_init() -> libc::c_int {
    unsafe{ return Curl_mbedtlsthreadlock_thread_setup();}
 }
 
 // 内部没有宏
extern "C" fn mbedtls_cleanup() {
    unsafe{  Curl_mbedtlsthreadlock_thread_cleanup();}
 }
 
 // 内部没有宏
extern "C" fn mbedtls_data_pending(
     mut conn: *const connectdata,
     mut sockindex: libc::c_int,
 ) -> bool {
     let mut connssl: *const ssl_connect_data = unsafe{ &*((*conn).ssl)
         .as_ptr()
         .offset(sockindex as isize) as *const ssl_connect_data};
     let mut backend: *mut ssl_backend_data =  unsafe{ (*connssl).backend};
     unsafe{ return mbedtls_ssl_get_bytes_avail(&mut (*backend).ssl)
         != 0 as libc::c_int as libc::c_ulong;}
 }
 ...
 // 1128 - done
     if cfg!(MBEDTLS_VERSION_NUMBER_LT_0X02070000){
         ...
         if cfg!(MBEDTLS_VERSION_NUMBER_GT_0X03000000){
             ...
             if  unsafe{mbedtls_sha256_ret(input, inputlen, sha256sum, 0 as libc::c_int)!= 0 as libc::c_int} {
...
extern "C" fn mbedtls_get_internals(
     ...
     let mut backend: *mut ssl_backend_data = unsafe{(*connssl).backend};
     unsafe{return &mut (*backend).ssl as *mut mbedtls_ssl_context as *mut libc::c_void;}
 }
