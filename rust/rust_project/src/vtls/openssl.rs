...
extern "C" fn passwd_callback(
    ...
    unsafe {
        ...
        if encrypting == 0 {
            let mut klen: i32 = curlx_uztosi(strlen(global_passwd as *mut libc::c_char));
            if num > klen {
                memcpy(
    ...
    unsafe {
        ...
        loop {
            ...
            while i < i_max {
                ...
                tv.tv_usec =
                    (tv.tv_usec as u32).wrapping_mul((i as u32).wrapping_add(2 as u32)) as i32;
...
extern "C" fn ssl_ui_reader(mut ui: *mut UI, mut uis: *mut UI_STRING) -> i32 {
    unsafe {
        ...
        return (UI_method_get_reader(UI_OpenSSL())).expect("non-null function pointer")(ui, uis);
    }
}
            ...
            match file_type {
                ...
                43 => {
                    ...
                    'fail: loop {
                        ...
                        if SSL_CTX_use_PrivateKey(ctx, pri) != 1 as i32 {
                            Curl_failf(
                                data,
...
extern "C" fn x509_name_oneline(
    ...
    unsafe {
        ...
        rc = X509_NAME_print_ex(bio_out, a, 0 as i32, ((3 as i32) << 16 as i32) as u64);
        BIO_ctrl(
            bio_out,
            115 as i32,
            0 as i64,
...
extern "C" fn verifyhost(
    ...
    unsafe {
        ...
        altnames = X509_get_ext_d2i(server_cert, 85 as i32, 0 as *mut i32, 0 as *mut i32)
            ...
            if dNSName as i32 != 0 || iPAddress as i32 != 0 {
                ...
                if i_0 >= 0 as i32 {
                    let mut tmp: *mut ASN1_STRING =
                        X509_NAME_ENTRY_get_data(X509_NAME_get_entry(name, i_0));
                    if !tmp.is_null() {
                ...
                if !(result as u64 != 0) {
                    ...
                    if peer_CN.is_null() {
                        ...
                        result = CURLE_PEER_FAILED_VERIFICATION;
                    } else if Curl_cert_hostcheck(peer_CN as *const libc::c_char, hostname) == 0 {
                        Curl_failf(
                        data,
...
extern "C" fn verifystatus(
    ...
    unsafe {
        ...
        'end: loop {
            ...
            if OCSP_basic_verify(br, ch, st, 0 as u64) <= 0 as i32 {
                Curl_failf(
                    data,
    ...
    unsafe {
        ...
        if ssl_ver != 0 && SSL3_RT_INNER_CONTENT_TYPE_flag {
            ...
            if content_type == 20 as i32 {
                msg_type = *(buf as *mut libc::c_char) as i32;
                msg_name = b"Change cipher spec\0" as *const u8 as *const libc::c_char;
            } else if content_type == 21 as i32 {
                msg_type = ((*(buf as *mut libc::c_char).offset(0 as isize) as i32) << 8 as i32)
...
extern "C" fn select_next_protocol(
    ...
    unsafe {
        ...
        while i.wrapping_add(keylen) <= inlen {
            ...
            i = i.wrapping_add((*in_0.offset(i as isize) as i32 + 1 as i32) as u32);
        }
        return -(1 as i32);
    }
}
...
extern "C" fn set_ssl_version_min_max(
    ...
    unsafe {
        ...
        #[cfg(CURL_DISABLE_PROXY)]
        let mut curl_ssl_version_min: i64 = (*conn).ssl_config.version;
...
extern "C" fn ossl_new_session_cb(mut ssl: *mut SSL, mut ssl_sessionid: *mut SSL_SESSION) -> i32 {
    unsafe {
        ...
        sockindex = sockindex_ptr.offset_from(((*conn).sock).as_mut_ptr()) as i32;
...
extern "C" fn load_cacert_from_memory(
    ...
    unsafe {
        ...
        if (*ca_info_blob).len > 2147483647 as size_t {
...
extern "C" fn ossl_connect_step1(
    ...
    unsafe {
        ...
        let hostname: *const libc::c_char = (*conn).host.name;
        let mut addr: in_addr = in_addr { s_addr: 0 };
        let ssl_version: i64 = (*conn).ssl_config.version;
        ...
        let ssl_crlfile: *const libc::c_char = (*data).set.ssl.CRLfile; /* CURLOPT_CAINFO_BLOB overrides CURLOPT_CAINFO */
        let mut error_buffer: [libc::c_char; 256] = [0; 256];
        let mut backend: *mut ssl_backend_data = (*connssl).backend;
        ...
        match ssl_version {
            0 | 1 | 4 | 5 | 6 | 7 => {
                req_method = TLS_client_method(); /* it will be handled later with the context options */
                sni = 1 as i32 != 0;
            }
            2 => {
                Curl_failf(
        ...
        /* OpenSSL contains code to work around lots of bugs and flaws in various
           ...
           must not be set.
        */
        ctx_options = (0x80000000 as u32 | 0x800 as u32 | 0x4 as u32 | 0x10 as u32 | 0x40 as u32)
            as ctx_option_t;
        ...
        if ((*conn).bits).tls_enable_alpn() != 0 {
             ...
             * format
             */
            if SSL_CTX_set_alpn_protos((*backend).ctx, protocols.as_mut_ptr(), cur as u32) != 0 {
                Curl_failf(
                    data,
        ...
        if !ssl_crlfile.is_null() {
            ...
            if lookup.is_null() || X509_load_crl_file(lookup, ssl_crlfile, 1 as i32) == 0 {
                Curl_failf(
                    data,
        ...
        if ((*conn).proxy_ssl[sockindex as usize]).use_0() != 0 {
            ...
            let mut handle: *mut SSL = (*(*conn).proxy_ssl[sockindex as usize].backend).handle;
            #[cfg(all(DEBUGBUILD, HAVE_ASSERT_H))]
...
extern "C" fn pubkey_show(
    ...
    unsafe {
        ...
        1 as i32 != BIO_ctrl(mem, 1 as i32, 0 as i64, 0 as *mut libc::c_void) as i32;
    }
}
#[cfg(USE_OPENSSL)]
extern "C" fn X509V3_ext(
    ...
    unsafe {
        ...
        while i < sk_X509_EXTENSION_num(exts) {
            ...
            if X509V3_EXT_print(bio_out, ext, 0 as u64, 0 as i32) == 0 {
...
extern "C" fn servercert(
    ...
    unsafe {
        ...
        #[cfg(not(CURL_DISABLE_PROXY))]
        let SSL_IS_PROXY_void = if CURLPROXY_HTTPS as u32 == (*conn).http_proxy.proxytype as u32
            && ssl_connection_complete as u32
                != (*conn).proxy_ssl[(if (*conn).sock[1 as usize] == -(1 as i32) {
                    0 as i32
                } else {
        ...
        if rc != 0 {
            ...
            if !(SSL_CONN_CONFIG_issuercert).is_null()
                || !(SSL_CONN_CONFIG_issuercert_blob).is_null()
            {
                ...
                issuer = PEM_read_bio_X509(fp, 0 as *mut *mut X509, None, 0 as *mut libc::c_void);
                if issuer.is_null() {
                    if strict {
                        Curl_failf(
                            data,
...
extern "C" fn ossl_connect_common(
    ...
    unsafe {
        ...
        if ssl_connect_1 as u32 == (*connssl).connecting_state as u32 {
            ...
            let timeout_ms: timediff_t = Curl_timeleft(data, 0 as *mut curltime, 1 as i32 != 0);
...
extern "C" fn ossl_recv(
    ...
    unsafe {
        ...
        if nread <= 0 as i64 {
            let mut err: i32 = SSL_get_error((*backend).handle, nread as i32);
            match err {
                0 => {}
                6 => {
                    if num == 0 as i32 {
