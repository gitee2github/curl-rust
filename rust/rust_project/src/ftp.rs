...
extern "C" fn ftp_endofresp(
    ...
    unsafe {
        if len > 3 as u64
            && (Curl_isdigit(*line.offset(0 as isize) as i32) != 0
                && Curl_isdigit(*line.offset(1 as isize) as i32) != 0
                && Curl_isdigit(*line.offset(2 as isize) as i32) != 0
...
#[cfg(DEBUGBUILD)]
extern "C" fn _state(mut data: *mut Curl_easy, mut newstate: ftpstate, mut lineno: i32) {
    unsafe {
        let mut conn: *mut connectdata = (*data).conn;
        let mut ftpc: *mut ftp_conn = &mut (*conn).proto.ftpc;
        if (*ftpc).state as u32 != newstate as u32 {
            Curl_infof(
                data,
                b"FTP %p (line %d) state change from %s to %s\0" as *const u8
                    as *const libc::c_char,
    ...
    unsafe {
        if (*ftpc).cwddone {
            ...
            if (*data).set.ftp_filemethod as u32 != FTPFILE_NOCWD as u32
                || !((*ftpc).dirdepth != 0
                    && *(*((*ftpc).dirs).offset(0 as isize)).offset(0 as isize) as i32
                        == '/' as i32)
            {
            } else {
                __assert_fail(
...
extern "C" fn ftp_state_use_port(mut data: *mut Curl_easy, mut fcmd: ftpport) -> CURLcode {
    unsafe {
      ...
      >(
        ...
        let mut string_ftpport: *mut libc::c_char = (*data).set.str_0[STRING_FTPPORT as usize];
        ...
        if !((*data).set.str_0[STRING_FTPPORT as usize]).is_null()
            && strlen((*data).set.str_0[STRING_FTPPORT as usize]) > 1 as u64
        {
            #[cfg(ENABLE_IPV6)]
            ...
            #[cfg(not(ENABLE_IPV6))]
            let mut addrlen: size_t = if 16 as u64 > strlen(string_ftpport) {
                16 as u64
            } else {
                strlen(string_ftpport)
            };
            ...
            if flag {
                ...
                if !ip_end.is_null() {
                    ...
                    strncpy(addr, ip_start, ip_end.offset_from(ip_start) as u64);
                }
                ...
                if !ip_end.is_null() {
                    #[cfg(ENABLE_IPV6)]
                    if inet_pton(10 as i32, string_ftpport, sa6 as *mut libc::c_void) == 1 as i32 {
                        ...
                        strncpy(addr, string_ftpport, ip_end.offset_from(ip_start) as u64);
                    }
                    #[cfg(not(ENABLE_IPV6))]
                    strncpy(addr, string_ftpport, ip_end.offset_from(ip_start) as u64);
                } else {
            ...
            match (*sa).sa_family as i32 {
                ...
                10 => {
                    r = inet_ntop(
                        ...
                        hbuf.as_mut_ptr(),
                        ::std::mem::size_of::<[libc::c_char; 1025]>() as u64 as curl_socklen_t,
                    );
                }
                _ => {
                    r = inet_ntop(
        ...
        while fcmd as u32 != DONE as u32 {
            if !(((*conn).bits).ftp_use_eprt() == 0 && EPRT as u32 == fcmd as u32) {
                /* if disabled, goto next */
                if !(PORT as u32 == fcmd as u32 && (*sa).sa_family as i32 != 2 as i32) {
                    /* PORT is IPv4 only */
                    match (*sa).sa_family as i32 {
                        2 => {
...
extern "C" fn control_address(mut conn: *mut connectdata) -> *mut libc::c_char {
    unsafe {
        ...
        not the ftp host. */
        #[cfg(not(CURL_DISABLE_PROXY))]
        if ((*conn).bits).tunnel_proxy() as i32 != 0 || ((*conn).bits).socksproxy() as i32 != 0 {
...
extern "C" fn ftp_state_pasv_resp(mut data: *mut Curl_easy, mut ftpcode: i32) -> CURLcode {
    ...
    unsafe {
        ...
        if (*ftpc).count1 == 0 as i32 && ftpcode == 229 as i32 {
            ...
            if !ptr.is_null() {
                ...
                if 5 as i32
                    == sscanf(
                        ...
                        &mut *separator.as_mut_ptr().offset(0 as isize) as *mut libc::c_char,
                        &mut *separator.as_mut_ptr().offset(1 as isize) as *mut libc::c_char,
                        &mut *separator.as_mut_ptr().offset(2 as isize) as *mut libc::c_char,
                        &mut num as *mut u32,
                        &mut *separator.as_mut_ptr().offset(3 as isize) as *mut libc::c_char,
                    )
                {
                    ...
                    if !ptr.is_null() {
                        (*ftpc).newport = (num & 0xffff as u32) as u16;
                        match () {
                            #[cfg(not(CURLDEBUG))]
                            _ => {
                                (*ftpc).newhost = Curl_cstrdup.expect("non-null function pointer")(
            ...
            let mut ip: [u32; 4] = [0 as u32, 0 as u32, 0 as u32, 0 as u32];
            let mut port: [u32; 2] = [0 as u32, 0 as u32];
            /*
            ...
            (*ftpc).newport = ((port[0 as usize] << 8 as i32).wrapping_add(port[1 as usize])
        ...
        match () {
            ...
            _ => {
                if ((*conn).bits).proxy() != 0 {
                    ...
                    rc = Curl_resolv(data, host_name, (*conn).port, 0 as i32 != 0, &mut addr);
                    ...
                    if addr.is_null() {
                        Curl_failf(
                            data,
                            b"Can't resolve proxy host %s:%hu\0" as *const u8
                                as *const libc::c_char,
                            host_name,
                            connectport as i32,
                        );
        ...
        if ((*data).set).verbose() != 0 {
            ...
            ftp_pasv_verbose(data, (*addr).addr, (*ftpc).newhost, connectport as i32);
        }
        Curl_resolv_unlock(data, addr); /* we're done using this address */
        #[cfg(not(CURLDEBUG))]
        Curl_cfree.expect("non-null function pointer")(
        ...
        match () {
            ...
            _ => {
                (*conn).secondaryhostname =
                    Curl_cstrdup.expect("non-null function pointer")((*ftpc).newhost);
            }
            #[cfg(CURLDEBUG)]
            _ => {
    ...
    unsafe {
        match ftpcode {
            213 => {
                ...
                if 6 as i32
                    == sscanf(
                        &mut *((*data).state.buffer).offset(4 as isize) as *mut libc::c_char,
        ...
        if (*data).set.timecondition as u64 != 0 {
            if (*data).info.filetime > 0 as i64 && (*data).set.timevalue > 0 as i64 {
...
extern "C" fn ftp_state_size_resp(
    ...
    unsafe {
        ...
        if ftpcode == 213 as i32 {
            ...
            number. */
            let mut start: *mut libc::c_char = &mut *buf.offset(4 as isize) as *mut libc::c_char;
            ...
            curlx_strtoofft(fdigit, 0 as *mut *mut libc::c_char, 0 as i32, &mut filesize);
...
extern "C" fn ftp_state_get_resp(
    ...
    unsafe {
        if ftpcode == 150 as i32 || ftpcode == 125 as i32 {
                ...
                && (*ftp).downloadsize < 1 as i64
            {
                ...
                if !bytes.is_null() {
                    ...
                    if !bytes.is_null() {
                        ...
                        curlx_strtoofft(bytes, 0 as *mut *mut libc::c_char, 0 as i32, &mut size);
                    }
                }
            } else if (*ftp).downloadsize > -(1 as i32) as i64 {
                size = (*ftp).downloadsize;
            }
            if size > (*data).req.maxdownload && (*data).req.maxdownload > 0 as i64 {
        ...
        } else if instate as u32 == FTP_LIST as u32 && ftpcode == 450 as i32 {
            ...
            return (if instate as u32 == FTP_RETR as u32 && ftpcode == 550 as i32 {
    ...
    if ftpcode != 0 {
        ...
        match (*ftpc).state as libc::c_uint {
            1 => {
                ...
                #[cfg(HAVE_GSSAPI)]
                if ((*data).set).krb() != 0 {
                    Curl_sec_request_prot(conn, b"private\0" as *const u8 as *const libc::c_char);
                    ...
                    if Curl_sec_login(data, conn) as u64 != 0 {
                        ...
                        Curl_infof(
                            data,
                            b"Authentication successful\0" as *const u8 as *const libc::c_char,
                        );
                    }
                }
...
extern "C" fn ftp_done(
    ...
    unsafe {
        ...
        match current_block_5 {
            8000488408776534573 => {
                /* by default, an error means the control connection is
                ...
                && *rawPath.offset(0 as isize) as i32 == '/' as i32
            {
                ...
                if !(*ftpc).cwdfail {
                    if (*data).set.ftp_filemethod as u32 == FTPFILE_NOCWD as u32 {
                        ...
                        pathLen = (pathLen as u64).wrapping_sub(if !((*ftpc).file).is_null() {
                            strlen((*ftpc).file)
                        } else {
                            0 as u64
            ...
            && !premature
        {
            ...
            if nread == 0 && CURLE_OPERATION_TIMEDOUT as u32 == result as u32 {
                Curl_failf(
                    data,
                ...
                Curl_conncontrol(
                    conn,
                    1 as i32,
                    b"Timeout or similar in FTP DONE operation\0" as *const u8
                        as *const libc::c_char,
                ); /* close */
            }
            if result as u64 != 0 {
            ...
            if (*ftpc).dont_check as i32 != 0 && (*data).req.maxdownload > 0 as i64 {
                ...
                Curl_conncontrol(
                    conn,
                    1 as i32,
                    b"Partial download with no ability to check\0" as *const u8
                        as *const libc::c_char,
                );
                return result;
            }
    ...
    unsafe {
        ...
        if !(*conn).bits.tcpconnect[1 as i32 as usize] {
            ...
            if Curl_connect_ongoing(conn) {
                ...
                result = Curl_proxyCONNECT(data, 1 as i32, 0 as *const libc::c_char, 0 as i32);
                return result;
            }
        ...
        match () {
            ...
            _ => {
                ...
                if (*conn).http_proxy.proxytype as u32 == CURLPROXY_HTTPS as u32
        ...
        if (*ftp).transfer as u32 <= PPTRANSFER_INFO as u32 {
            ...
            if (*ftpc).wait_data_conn as i32 == 1 as i32 {
                ...
                if result as u32 == CURLE_OK as u32 && (*data).req.maxdownload >= 0 as i64 {
    ...
    unsafe {
        ...
        if !last_slash.is_null() {
            ...
            match () {
                ...
                _ => {
                    (*wildcard).pattern =
                        Curl_cstrdup.expect("non-null function pointer")(last_slash);
                }
                #[cfg(CURLDEBUG)]
                _ => {
        ...
        'fail: loop {
            if ftpwc.is_null() {
                ...
                if ((*ftpwc).parser).is_null() {
                    ...
                    (*wildcard).dtor =
                        Some(wc_data_dtor as unsafe extern "C" fn(*mut libc::c_void) -> ());
                    if (*data).set.ftp_filemethod as u32 == FTPFILE_NOCWD as u32 {
                    ...
                    if !(result as u64 != 0) {
                        match () {
                            ...
                            _ => {
                                (*wildcard).path =
                                    Curl_cstrdup.expect("non-null function pointer")((*ftp).path);
                            }
                            #[cfg(CURLDEBUG)]
                            _ => {
        ...
        loop {
            match (*wildcard).state as u32 {
                ...
                3 => {
                    ...
                    if (*finfo).filetype as u32 != CURLFILETYPE_FILE as u32 {
                        (*wildcard).state = CURLWC_SKIP;
                    } else {
                        if (*finfo).flags & ((1 as i32) << 6 as i32) as u32 != 0 {
                ...
                5 => {
                    ...
                    (*wildcard).state = (if (*wildcard).filelist.size == 0 as u64 {
                        CURLWC_CLEAN as i32
                    } else {
                ...
                7 | 6 | 0 => {
                    if ((*wildcard).dtor).is_some() {
                        ((*wildcard).dtor).expect("non-null function pointer")(
                            (*wildcard).protdata,
                        );
                    }
                    return result;
    ...
    unsafe {
        ...
        match (*data).set.ftp_filemethod as u32 {
            2 => {
                ...
                if pathLen > 0 as u64
                    && *rawPath.offset(pathLen.wrapping_sub(1 as u64) as isize) as i32 != '/' as i32
                {
            ...
            3 => {
                ...
                if !slashPos.is_null() {
                    ...
                    strncpy(*((*ftpc).dirs).offset(0 as isize), rawPath, dirlen);
            ...
            1 | _ => {
                ...
                if dirAlloc > 0 as i32 {
                    ...
                    loop {
                        ...
                        let mut compLen: size_t = slashPos.offset_from(curPos) as size_t;
                        /* path starts with a slash: add that as a directory */
                        if compLen == 0 as u64 && (*ftpc).dirdepth == 0 as i32 {
                            compLen = compLen.wrapping_add(1);
                        }
